...
- **Expected:** (a) Four shimmer rows render while a segment loads; no "Loading…" caption. (b) Empty Upcoming reads "Nothing reserved yet." over "Your week is open." with a "Book a class" ghost pill; empty Past reads "Your first class will show here." (c) Past is read from `GET bookings/me/?scope=past` 20 rows per page, newest first, with no day headers and no Cancel links; scrolling within 10 rows of the end loads the next page behind a single shimmer row (`reservations_page_loaded` with `page_index` and `count` each time), rows are deduplicated by id, and the last page stops requesting. A failed later page keeps the rows already shown and appends an `InlineError` whose retry re-requests that page; scrolling alone does not retry. Pull-to-refresh re-reads the visible segment (Past resets to page one and discards a stale in-flight page); with nothing loaded a failure shows `FullScreenError` (CONNECTION vs SERVER), with content on screen it keeps the content and shows the `ErrorSnackbar` refresh-failed toast.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/networking/ArcanaApiClient.kt (`myUpcoming`, `myPast`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/BookingDto.kt (`MyUpcomingDto`, `MyPastDto`)
- **Platforms:** shared
...
- **Steps:** From any tab root, navigate into a non-tab screen (ClassDetail, StudioSelection, MyBookings, EditProfile, ConciergeRequest, Search).
...
- **Expected:** An `ArcanaSheet` with the instructor's monogram and name and their bio, or "No bio yet." when blank; `instructor_tapped` fires with `profile_id` and `source=studio_page`. No schedule link and no feedback link yet (reviews phase).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageScreen.kt (`InstructorSheet`, `InstructorRow`)
- **Platforms:** shared
