...
**228 entries across 14 areas** (keep this line and the runbook's pinned count in sync when adding an entry): LAUNCH 6, AUTH 15, SIGNUP 24, HOME 20, SCHED 19, CLASS 26, FAV 9, PROFILE 26, CONCIERGE 4, DEVSET 11, NAV 13, ERR 22, TEL 23, PLAT 10. CONCIERGE was added during the 2026-08-11 completeness sweep (the Concierge Request screen, reached from Profile, had no coverage in the original 13-area plan); a new top-level surface warrants a new area section rather than squeezing into a neighbor. ERR grew from 20 to 22 on 2026-08-16 (`feature/error-states-completion`): ERR-21 (Home refresh-failed toast) and ERR-22 (client request timeout) are new surfaces this branch created; every other area's count is unchanged — that work rewrote several existing entries' Expected text in place but added no new surfaces outside ERR. NAV grew from 12 to 13 on 2026-08-19 (`fix/NAV-13-pre-auth-system-back`): NAV-13 covers the new confirm-to-leave dialog, and NAV-12's Expected was rewritten in place because it had encoded the defect as intended behavior. TEL grew from 21 to 23 on 2026-08-22 (`fix/HAZARD-posthog-release-only-prod`): TEL-22 (analytics gate) and TEL-23 (Sentry environment tagging) cover the new gate, and TEL-20's Expected was corrected since a Debug build no longer reaches PostHog at all. AUTH grew from 14 to 15 on 2026-08-24: AUTH-15 covers soft-keyboard CTA reachability, which no entry had claimed, and which a run had mis-scored as an app bug because the layout dump omits the keyboard. PLAT grew from 9 to 10 on 2026-08-24: PLAT-10 records the measured 48dp touch-target floor, after a card claimed icon controls were 36dp targets.
...
- **Steps:** Two halves, and the difference is the whole point. (a) Make the refresh endpoint answer 401/403 for a live session (e.g. restart the local server with a different `SECRET_KEY` so existing tokens no longer verify), then trigger any authenticated request. (b) Make the stored refresh token unreadable instead, with the server healthy: clear the Keychain / EncryptedSharedPreferences externally while a session is active, then trigger an authenticated request.
- **Expected:** (a) signs the member out. `forceLogout("refresh_error")` records `SecureStorageDiagnostics.lastFailureFor(REFRESH_TOKEN_KEY)`, fires `telemetry.forcedLogout(cause, osStatus, storageOp, storageKey)`, reports a `ForcedLogoutSignal` nonfatal to Sentry BEFORE `telemetry.reset()` (so the member is still attached), clears tokens, and flips `isAuthenticated` false. (b) does NOT sign the member out: a null storage read is unproven, exactly like a timeout or a 5xx, so the session is kept and the next request retries. It reports `auth_refresh_failed{outcome:"no_stored_refresh"}` carrying `storage_os_status`/`storage_op`. **A genuinely unrecoverable token therefore leaves an authed shell whose requests keep failing** rather than an unexplained logout; the member's exit is the Profile tab's own Sign out (AUTH-11). That trade is deliberate (Cole, 2026-08-16): only an affirmative rejection may end a session.
...
- **Steps:** Four combinations, each from a cold start after setting the base URL in Developer Settings (DEVSET-01/AUTH-13) and force-stopping the app: (a) Debug build on `https://api.arcana.fit`, (b) Debug build on a local/tunnel URL, (c) release build on a local/tunnel URL, (d) release build on `https://api.arcana.fit`. Watch for outbound requests to the PostHog host.
- **Expected:** Only (d) contacts PostHog (a `/array/<key>/config` fetch, then `/flags` and batched `/batch` posts). (a), (b) and (c) send nothing at all: the SDK is never initialized and `NoopAnalytics` is bound, because `TelemetryGate.shouldReportAnalytics` requires `!isDebugBuild && environment == prod` and the environment is resolved from the persisted override BEFORE Koin starts. The Debug-build console echo (TEL-20) still fires in every case, so events remain observable while developing. Session replay follows PostHog and is likewise absent in (a)-(c).
...
- **Steps:** Run a Debug build against a local server and trigger a handled error (e.g. a failing request on Home), then repeat on a release build against prod.
- **Expected:** Both reach Sentry: unlike PostHog, Sentry is deliberately never gated, so dev and regression-run issues are still captured. Each report carries the SDK's native `environment` set from the same classifier — `local-debug` in the first case, `prod` in the second (`sentryEnvironment` appends `-debug` for a debug build) — which is what new-issue alert rules scope on.
