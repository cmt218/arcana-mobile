...
**264 entries across 17 areas** (keep this line and the runbook's pinned count in sync when adding an entry): LAUNCH 6, AUTH 16, SIGNUP 24, HOME 21, SCHED 23, CLASS 29, FAV 9, PROFILE 27, CONCIERGE 4, DEVSET 12, NAV 13, ERR 22, TEL 27, PLAT 14, SEARCH 5, DISCOVER 7, REVIEW 5. CONCIERGE was added during the 2026-08-11 completeness sweep (the Concierge Request screen, reached from Profile, had no coverage in the original 13-area plan); a new top-level surface warrants a new area section rather than squeezing into a neighbor. ERR grew from 20 to 22 on 2026-08-16 (`feature/error-states-completion`): ERR-21 (Home refresh-failed toast) and ERR-22 (client request timeout) are new surfaces this branch created; every other area's count is unchanged — that work rewrote several existing entries' Expected text in place but added no new surfaces outside ERR. NAV grew from 12 to 13 on 2026-08-19 (`fix/NAV-13-pre-auth-system-back`): NAV-13 covers the new confirm-to-leave dialog, and NAV-12's Expected was rewritten in place because it had encoded the defect as intended behavior. TEL grew from 21 to 23 on 2026-08-22 (`fix/HAZARD-posthog-release-only-prod`): TEL-22 (analytics gate) and TEL-23 (Sentry environment tagging) cover the new gate, and TEL-20's Expected was corrected since a Debug build no longer reaches PostHog at all. AUTH grew from 14 to 15 on 2026-08-24: AUTH-15 covers soft-keyboard CTA reachability, which no entry had claimed, and which a run had mis-scored as an app bug because the layout dump omits the keyboard. PLAT grew from 9 to 10 on 2026-08-24: PLAT-10 records the measured 48dp touch-target floor, after a card claimed icon controls were 36dp targets. PLAT-11 was added 2026-08-25 (`fix/UI-textlink-optical-centring`): no entry covered optical centring at all, and the fix turned on which of the two helpers a component needs, so the distinction is recorded rather than left to the next reader to rediscover. PROFILE-02's Expected was rewritten in place on 2026-08-26 (`profile-credits-empty-state`) for the same reason NAV-12's was: it had encoded the defect as intended behavior, describing the endless Credits shimmer as the expected no-wallet rendering. No count changed. AUTH grew from 15 to 16 on 2026-08-24 (`App-review accounts sign in against staging`): AUTH-16 landed with the entry but not with this count line, which is why the run of 2026-08-27 reconciled 230 entries against a doc claiming 229 — the count was corrected on 2026-08-28, and the entry itself was already correct. HOME-09, CLASS-21 and SIGNUP-06 had their Expected text corrected in place on 2026-08-28 after that run read each one against its Source. SEARCH (5 entries) was added 2026-08-29 (`class-search`): the full-screen Book-tab search takeover is a new top-level surface (same call as CONCIERGE); SCHED grew 19→20 (SCHED-20, the header entry point), TEL 23→24 (TEL-24, the search taxonomy), and NAV-04's Steps gained Search in its pushed-destination list. NAV-01 and AUTH-02's Expected were rewritten in place on 2026-09-05 (`feature/polish-0-foundations`): NAV-01 now describes the single travelling Lime dot + icon bounce that replaced the old static per-item dot, and instant (no-fade) tab-to-tab swaps; AUTH-02 documents the PrimaryCta press/release feedback (97% scale, darkened fill, kicked well). No count changed. DEVSET grew from 11 to 12 on 2026-09-05 (`feature/polish-0-foundations`): DEVSET-12 covers the new founders-only Design system reference page, a new surface reached from Developer Settings rather than a variant of the base-URL controls, so it takes its own entry inside DEVSET rather than a new area. PLAT grew from 11 to 12 on 2026-09-06 (`feature/polish-1-atmosphere`): PLAT-12 covers the living atmosphere surface that replaced flat Stone on every Stone root; it is one shared surface with one set of gates, so it is a platform entry rather than a per-screen Expected rewrite. DEVSET-12's Expected gained the page's new Atmosphere section in place. NAV-01, NAV-04, CLASS-01, CLASS-08, CLASS-14, CLASS-18, PROFILE-05, SCHED-12, ERR-02, ERR-05, ERR-21, PLAT-08 and PLAT-09 had their Expected (and several their Source) rewritten in place on 2026-09-06 (`feature/polish-2-navigation`): the class-detail push-and-recede, the shared `ArcanaSheet` now used by both the booking and cancel sheets, the floating Paper pill that replaced Android's docked Scaffold tab bar, and the fade-and-rise treatment now applied to every transient snackbar and the favourites nudge. DEVSET-12's Expected also gained the page's new Navigation section, between Motion and Tab bar. No count changed. SCHED grew from 20 to 22 on 2026-09-07 (`feature/polish-3-schedule-pager`, landed on the `feature/polish-2-navigation` branch per Cole's instruction): SCHED-21 (unloaded-day skeleton rows) and SCHED-22 (the per-row rise stagger after a page settles) are new; SCHED-03, SCHED-04, SCHED-05, SCHED-06, SCHED-12, SCHED-13 and SCHED-18 had their Expected and/or Source rewritten in place for the Book tab's class list becoming a `HorizontalPager` (drag-follows-finger, spring settle, one gliding rail pill, per-boundary haptic tick) with a pinned header above it; ERR-03's Expected and Source were also corrected for the same reason. No other area's count changed. Phase 4's tactile pass (`feature/polish-2-navigation`, 2026-09-07) rewrote Expected text in place on DEVSET-12, ERR-01, ERR-03, ERR-05, ERR-06, SCHED-03, SCHED-09, SCHED-10, SCHED-11, SCHED-12, SCHED-13, SCHED-22, FAV-02, FAV-07, CLASS-01, CLASS-06, CLASS-08, SEARCH-02, SEARCH-03, HOME-07, HOME-13, HOME-14, HOME-15, PLAT-10 and PLAT-11 for the new press/depth feedback on existing controls and the decision-H translucent-surface token (`PaperTranslucent`, `theme/AppColors.kt`); DEVSET-12 also gained the page's new Tactile section and, while in there, a description of its pre-existing Schedule section, which `feature/polish-3-schedule-pager` had added to the page without updating this entry. No new entries were added and no count changed. CLASS grew from 26 to 27 on 2026-09-07 (`feature/polish-5-gestures` ceremonies trial, landed on the `feature/polish-2-navigation` branch per Cole's instruction): CLASS-27 covers the in-place booking confirmation (the slide well's arrow→check pop, the sticky CTA's one-time Lime pulse, and the ~550ms sheet hold before it closes), a new surface; CLASS-08's and CLASS-18's Expected were rewritten in place for the slide-to-book and hold-to-cancel controls that now gate the booking sheet's CONFIRM and the cancel sheet's CANCEL BOOKING behind the `BookingGestures.enabled` trial switch and the TalkBack/VoiceOver accessibility fallback, and DEVSET-12's Expected gained the page's new Gestures section (the demo slide and hold controls, between Tactile and Motion). The booking ViewModel and every request are unchanged: the gestures call the same confirm/cancel lambdas the taps do. No other area's count changed. The Book-tab filters were rebuilt into anchored popovers on 2026-09-08 (`feature/polish-2-navigation`): the four expandable panels (Favorites / All Studios / Time / Modalities) now open as a floating rounded-rectangle card that OVERLAYS the day pager instead of pushing it down, the All-Studios list uses a new compact single-line row (`CompactStudioFilterRow`) rather than the tall `StudioAccordionCard` (which is unchanged and still used on the favorites StudioSelection screen (FAV-07) and the Design-system reference (DEVSET-12)), the scope toggle's selected fill moved Ink→Moss, and the Time/Modalities pills gained a visible soft unselected outline (`Outline`, 29% Ink). SCHED-09, SCHED-10, SCHED-11 and SCHED-13 had their Expected (and SCHED-09/13 their Source) rewritten in place; the presentation-only change added and removed no entries and no count changed. PROFILE grew from 26 to 27 and TEL from 24 to 25 on 2026-09-09 (`feature/discover-mobile`, Reservations phase): PROFILE-27 covers the new You-tab "Reservations" row and TEL-25 the reservations taxonomy; CLASS-22 and CLASS-23 were rewritten in place for the My Bookings screen becoming Reservations (Upcoming | Past segments, day headers, scoped `bookings/me/` reads, paged Past, shimmer, empty states, pull-to-refresh, the shared cancel sheet), CLASS-18's Expected and Source moved to the extracted `CancelReservationSheet` with its "Cancel reservation?" copy, and HOME-10 and HOME-13 had their copy corrected ("Nothing reserved yet.", "Reservations"). CLASS grew from 27 to 28 in the same branch (addresses phase): CLASS-28 covers the shared `AddressRow`/`AddressLink` maps sheet on class detail, reservation rows and the Home next-up card, which had no entry (the old location row was deliberately non-tappable), and TEL-25 gained `address_tapped`. On 2026-09-15 (phase 3 leftovers, mobile only) SCHED-05, SCHED-13, FAV-02, FAV-07, FAV-08, FAV-09, PROFILE-08 and DISCOVER-01 gained addenda for brand grouping (overview `brands[]`, favorites `brand_slugs`) and the per-category glyphs; no count changed. DISCOVER (6 entries) was added in the same branch (Discover phase): the fourth tab is a new top-level surface (same call as CONCIERGE and SEARCH); SCHED grew 22 to 23 (SCHED-23, the brand scope chip a studio page's "See schedule" hands to the Book tab), TEL 25 to 26 (TEL-26, the Discover taxonomy), and NAV-01 and NAV-04 had their Expected rewritten in place for the four-tab bar and the new pushed destination. REVIEW (5 entries) was added 2026-09-15 (`feature/reviews-mobile`, phase 4): the review card, the Home prompt rules, the feedback feed and its entry points are a new surface reached from four tabs (same call as CONCIERGE, SEARCH and DISCOVER); HOME grew 20 to 21 (HOME-21, the prompt card under Next up), CLASS 28 to 29 (CLASS-29, the card and the feedback links on class detail) with CLASS-23 gaining the past rows' inline step one, DISCOVER 6 to 7 (DISCOVER-07, the Member feedback row and the studio page's "What members say" links) with DISCOVER-05 rewritten in place for the sheet's feedback link, and TEL 26 to 27 (TEL-27, the reviews taxonomy). NAV-04's pushed-destination list gains FeedbackFeed. On 2026-09-18 (same branch, Cole's UI pass) HOME-11, HOME-21, CLASS-23, CLASS-29, TEL-27, REVIEW-01 and REVIEW-03 had their Expected (and several their Source) rewritten in place for the Moss review card, the inline Reservations flow that keeps asking the next question, MossLight rules, the end-of-feed marker and the all-studios feed losing its stats line; no entry was added and no count changed. A second pass the same night (Cole chose the tonal well over the Moss card) rewrote HOME-21, CLASS-23, CLASS-29, REVIEW-01, REVIEW-02, REVIEW-03, DISCOVER-05 and DISCOVER-07 in place again: one tonal well for every review surface, feed items as cards with the fixed five-column answers strip, contextual "What members say" hints on class detail, the card that stays open and rests as "FEEDBACK SAVED", answers that show before they save, and the one-to-three line comment field. Still 262 entries. A third pass on 2026-09-19 rewrote HOME-21, CLASS-29, REVIEW-01, REVIEW-02, REVIEW-03, SCHED-05 and SEARCH-02 in place: the once-a-day Home cadence, the pinned in-progress card, the `SettlingCta` saved state that never greys, pips and the averages card in the feed, and the end-of-list marker losing its three dots on the Book tab, Search and the feed. Still 262 entries. A fourth pass the same day took the count to 263: PLAT-13 is new (the iOS keyboard prewarm; a platform behaviour with its own measurement, so its own entry), and HOME-06, HOME-21, CLASS-23, REVIEW-01, REVIEW-03, CONCIERGE-01 and CONCIERGE-03 were rewritten in place for the save-failed notice and kept drafts, the three-day "Not now" pause, Home treating a failed reservations read as a failure, averages only above one review with the Again ring, and the concierge field and never-grey Send. PLAT grew from 13 to 14 on 2026-09-20 (`fix/sentry-corrupt-report-preflight`): PLAT-14 covers the startup pre-flight that deletes an unusable saved crash report before `SentrySDK.start` reads it; it is a launch-blocking platform behaviour with its own planting recipe, so it takes its own entry rather than an addendum to LAUNCH. The count went 263 to 264.
...
- **Expected:** Pressing the CTA scales it to 97% and darkens the fill; release springs back with a small kick of the Lime well. CTA swaps to a Moss pill with a Lime spinner while in flight (`AuthUiState.Loading`). On success `AuthViewModel.uiState` becomes `Success`, `ArcanaApiClient.isAuthenticated` flips true, and the app transitions into the authenticated shell (AuthFlowRoot renders inert Stone for one frame during the swap on iOS; App.kt swaps directly on Android).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/auth/AuthScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Buttons.kt (`PrimaryCta`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Pressable.kt (`pressable`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/auth/AuthViewModel.kt
- **Platforms:** shared
...
- **Expected:** A "Next · {relative time}" section rule appears (e.g. "Next · in 18min", "Next · in 3h", or "Next · Mon 6:00am" depending on how far out the class is) followed by a dark Moss card showing the studio (and spot label if assigned), a booking-status pill, the studio-local start time with am/pm (never the device timezone), the class name, and a studio/location/duration meta line. The card now carries a `cardShadow` and a warm one-pixel highlight along its top edge (`innerHighlight`) instead of a flat Moss fill with no depth.
...
### HOME-13 — "See all" link opens Reservations
- **Steps:** From Home, tap the "See all" text link below the upcoming rows.
- **Expected:** `onSeeAllBookings` fires, navigating to the Reservations screen (`MyBookings(source = "home")`, which emits `reservations_opened` with `source=home`) regardless of whether there are 0, few, or many upcoming reservations — the link always renders in the Success state. The `TextLink` itself now dims to 70% alpha while pressed (no scale) instead of showing no press feedback at all — every other `TextLink` call site (My Bookings' "Cancel", Profile's "Manage", Signup's "Continue anyway"/"Log in") gets the same behavior for free, since it's one shared component.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/home/HomeScreen.kt (lines 276-285), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Buttons.kt (`TextLink`)
- **Platforms:** shared
...
- **Expected:** `onOpenClass(session.id)` fires with that specific booking's session id, navigating into Class Detail for that class. Both controls now press before the navigation fires: the Next Up card scales to 98%, an upcoming row scales to 99% — no ripple on either.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/home/HomeScreen.kt (lines 216-220, 264-271, `NextUpCard`, `UpcomingRow`)
- **Platforms:** shared
...
- **Expected:** The dark card at the bottom reads "{N} classes remaining." plus a line "{N}-week streak. Keep it going." beneath it. **The second line is the whole entry** — the credits line alone is HOME-16's zero-streak branch, and no seeded account has a nonzero streak (it is server-computed from real attendance), so this entry stays BLOCKED on that fixture gap until one exists. Do not PASS it on the credits line. The card (Ink, not tappable) now carries a `cardShadow` and the same warm top-edge `innerHighlight` as the Next Up card. **Home has no `Paper`/`PaperTranslucent` surface anywhere** (its two cards are Moss and Ink) — decision H is not visible on this screen, and a driver should not read Home as having changed color.
...
### HOME-21 — Review prompt card under Next up: one card, "Not now", the seven-day pause
- **Steps:** As a member with a completed (or confirmed and ended) reservation that ended between 30 minutes and 7 days ago and whose class type with that instructor at that studio the member has not reviewed (locally the QA member's newest completed reservation), open Home. Tap Yes; answer the intensity; tap Done. Reload Home. Then, with two further unreviewed recent reservations, tap "Not now" twice.
- **Expected:** Home reads `GET bookings/me/?scope=upcoming` (studio-cancelled rows are dropped from Next up and Upcoming) and, when its `review_prompt` is non-null, shows one `ReviewCard` directly under the Next up block, in the tonal well every review surface shares (`Modifier.tonalWell`: a Moss tint over the atmosphere, a soft MossLight edge, 20dp corners, NO shadow, so it reads as recessed and never as a second Next up card): a Moss caps line "YESTERDAY · SOTO METHOD" (Today / Yesterday / "Sat Sep 12", then the brand) over "Sculpt 50 with Jess" (the instructor clause omitted when unknown), asking "Would you take it again?" with Yes / Maybe / No, each led by its mark (green check, amber warning, red minus; REVIEW-01), and a "Not now" link. Answers are outlined pills (transparent, MossLight edge) until picked; the pick fills Moss with Stone type. No Surface or white fill appears anywhere on the card. The first tap POSTs `bookings/<id>/review/` (the review exists from then on), the card moves to "How hard was it?" (one, two or three flames) and then to the three 1 to 5 rows (instructor name, class type, studio) plus the public comment field; Done saves a changed comment and the card disappears. After a reload the prompt has moved to the next unreviewed class or is gone. Home asks at most once a day, and the rule lives on the server (`reviews/prompting.py` `quiet_until`): after the member creates a review from ANY surface, and after every "Not now", no prompt is offered for 24 hours; a "Not now" that directly follows another one, with no review created between them and however far apart, quiets Home for THREE days from the later one, and so does every further "Not now" in that run (after the pause the next prompt, dismissed again, pauses three days again); a review from any surface ends the run, so the "Not now" after it costs only the day. "Not now" hides the card at once and POSTs `review/dismiss/`, whose answer is therefore normally no prompt; that reservation never prompts again. If that POST is lost (no signal), the card still stays away for the rest of the session instead of popping back on the next refresh. A card the member has started answering is pinned: a refresh or a return to Home keeps it until Done (the server stops offering it the moment step one lands). Tapping Done presses the pill in, fires the confirm haptic and shows "FEEDBACK SAVED" with the check popping into the Lime well for about 650ms, then the card leaves; the pill stays Moss throughout and never greys. The card never appears in the Loading state and at most one shows. To see a prompt again locally, re-run `tmp/qa_reviews_seed.py` (it clears the member's reviews and dismissals).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/home/HomeScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/home/HomeViewModel.kt (`dismissReviewPrompt`, `reviewStarted`, `clearReviewPrompt`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCard.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCopy.kt (`reviewPromptEyebrow`, `classWithInstructor`), sharedLogic/src/commonTest/kotlin/org/arcana/mobile/booking/HomeViewModelTest.kt
- **Platforms:** shared
...
- **Steps:** On Schedule, tap a day chip in the horizontal day rail (not the currently selected day). Separately, tap one chip and — before the pager finishes animating to it — tap a different chip.
- **Expected:** The selected date updates immediately (Display header month can change) and a `scheduleDayChanged` telemetry event fires with direction forward/backward and day offset from today. The `HorizontalPager` animates to the tapped day on a settle spring (`Springs.settle()`); the day rail's single Moss selection pill glides from its old chip to the new one over that animation rather than jumping, stays fully opaque throughout, and the rail auto-scrolls so the target chip is fully visible if it wasn't already. Tapping a second, different chip while the first tap's pager animation (or an in-flight swipe fling) is still settling is honoured, not dropped: the pager ends up on the newest tapped day. If the tapped day's page 1 isn't cached under the current filter set, its page shows skeleton rows (SCHED-21) while it fetches; the pinned header (month title, search pill, day rail, filter section) stays interactive throughout. Every chip's own fill (behind the sliding Moss pill) is now `PaperTranslucent` rather than opaque `Paper`, composed with the distance-based coverage fade it already had (decision H's token; opaque if Cole rules the other way).
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`selectDay`, `ensureSelectedDayLoaded`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`SuccessContent`, the `selectedIndex` → `pagerState.animateScrollToPage` `LaunchedEffect`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayRail.kt (`DayRail`, `DayChip`, `ChipFill`, `railScrollTargetPx`)
- **Platforms:** shared

### SCHED-04 — Day picker: horizontal swipe over the class list
- **Steps:** On the class list body (not the pinned header above it), drag left toward later days or right toward earlier days, then release. Separately: scroll partway down a day's list, swipe forward two or more days, then swipe back to the original day.
- **Expected:** The page follows the finger continuously as it is dragged — there is no minimum-distance threshold, unlike the old 56dp drag gesture that preceded this pager. Releasing settles the current page on the nearest day via a spring (`Springs.settle()`), not a fixed-duration tween. While the finger (or the settling fling after release) drives the scroll, `Haptics.tick()` fires once per page boundary crossed; a chip-tap-driven jump across several days (SCHED-03) does not replay a tick per page it passes through. The day rail's single Moss selection pill is bound to the pager's fractional position (`currentPage + currentPageOffsetFraction`): it glides continuously between chips as the page is dragged, stays fully opaque throughout (the chips fade under it, never the pill), and the rail auto-scrolls to keep the settled day's chip in view. Once the page settles on a new day, `scheduleDayChanged` fires with `method = "swipe"` (direction/day-offset as in SCHED-03); `selectDay` no-ops the telemetry when the settled date already matches the selected one, so a chip-tap-driven scroll settling on its own target does not double-fire. A day whose page 1 hasn't loaded shows skeleton rows (SCHED-21) — the old 0.4→1.0 200ms cross-fade is gone. **Per-day scroll position is not preserved across a swipe:** each `DayPage` owns its own `LazyListState` and the pager keeps only the current page and its immediate neighbours composed, so swiping two or more days away from a day and back returns that day to the top rather than where it was scrolled — this is accepted behaviour, not a regression to file. `dayAfterSwipe` (`:sharedLogic`) still exists with its own unit tests, but the screen no longer calls it; the pager's own drag/settle physics fully replaced it.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`SuccessContent`, `HorizontalPager`, the `pagerState.settledPage`/`fingerDriven`/haptics `LaunchedEffect`s), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayRail.kt (`DayRail`, `railPillOffsetPx`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`DayPage`, `rememberLazyListState`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`selectDay`)
- **Platforms:** shared

### SCHED-05 — Infinite scroll pagination (load more)
- **Steps:** On a day with more than one page of sessions, scroll its list down until within 10 items of the bottom.
- **Expected:** `ScheduleViewModel.loadMore()` fires automatically, guarded so it only runs when page 1 is loaded, a `nextCursor` exists, and no page is already in flight. Only the current page's own scroll can trigger it — each `DayPage`'s scroll-position collector is gated on `isCurrent`, since `loadMore` always acts on the ViewModel's selected day, not whichever page happens to be adjacent in the pager. New sessions append (deduped by session id), a compact loader shows at that page's footer while fetching, and once `nextCursor` is null the list shows the `EndOfListMarker` instead: one centred caps line, "THAT'S EVERYTHING FOR <WEEKDAY>", with no dots or other ornament above it.
- **Expected (2026-09-15 addendum):** Row meta reads BRAND · LOCATION with the brand name from the overview (`brandNames`, location id keyed) and the location short label stripped of the brand, e.g. "ID HOT YOGA · FIDI"; a location the overview does not place falls back to the site row's name as before.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`loadMore`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`LOAD_MORE_LOOKAHEAD` `LaunchedEffect`, `isCurrent` gate, "load-more"/"end-of-day" items), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`LOAD_MORE_LOOKAHEAD`, `EndOfListMarker`)
- **Platforms:** shared
...
- **Expected:** Once the day's page 1 has loaded (`dayLoaded == true` and no `dayError`), that day's page shows "No classes match your filters for this day." in place of any band headers or rows. Reach this ONLY via a genuinely empty result — do not use a forced server failure to test it: since `bodyOrThrow`, a 5xx on the day fetch now correctly reaches ERR-03's `InlineError` card instead. (Before `bodyOrThrow`, a 5xx with a JSON error body silently deserialized into an empty page and landed here instead of an error state — a server outage looked identical to a genuinely empty day.)
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`DayPage`, "empty" item), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/networking/ArcanaApiClient.kt (`fetchSessionsPage`, `bodyOrThrow`)
- **Platforms:** shared
...
- **Steps:** Open the "MODALITIES" filter pill, tap one or more curated categories in the flat list, tap DONE (or tap the ✕ on a resulting chip). **Drive the narrowing half in ALL STUDIOS scope, against a real synced studio.** The `regression-*` studios all carry a studio-level `yoga` category override from `seed_regression` while every seeded template's modality is `Strength`, and `classes/filters.py` matches a session whose studio holds the category — so "Yoga" legitimately returns every seeded session, and in FAVORITES scope (whose locations are only those studios') the filter can never appear to narrow. That is the fixture, not a defect.
- **Expected:** Each toggle updates `selectedModalitySlugs` immediately and marks `refreshingFilters = true` (dims the list, shows a compact loader between chips and list); after a 250ms debounce the server-side refetch settles with sessions narrowed to the selected categories. Picks render as removable chips below the filter pills; the pill itself only appears when `availableModalities` is non-empty for the current window. The MODALITIES pill presses (97% scale); its selected/active fill is now **Moss** (changed from Ink) and, when unselected, it shows a visible **Ash** outline (changed from a faint Mist hairline) so it reads as a bordered pill. The panel is now an **anchored popover card** that overlays the schedule without pushing the day pager down (see SCHED-13): it drops open directly under the controls, is never wider than the controls, and closes on a re-tap of MODALITIES, a tap on the schedule outside the card, or DONE. The modality options inside remain the flat `StudioLocationRow` multi-select list. Each removable chip carries a soft shadow and its × well presses at 90% scale.
- _Note added after run 2026-09-04: this was recorded FAIL ("modality filter does not narrow in Favorites scope") and adjudicated a seed-fixture artifact. Verified server-side: `category=cycle` with the same `location_id` list returns 0 rows, so the narrowing itself works._
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`toggleModality`, `removeModality`, `onFiltersChanged`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`FilterControls`, `FilterPopoverOverlay`, modality panel, `FilterPill`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/FilterChip.kt
- **Platforms:** shared
...
- **Expected:** A preset applies immediately (`onApply`); a custom range only commits a `TimeFilter` if it's narrower than the full span (a full-span selection clears the filter instead). The active filter renders as a removable chip and narrows sessions server-side via `startTimeGte`/`startTimeLte` on the next debounced refetch. The TIME pill presses (97% scale); its selected fill is now **Moss** (changed from Ink) with a visible **Ash** outline when unselected (changed from Mist). Each preset pill (`SelectablePill`) keeps its Moss selected fill (`controlShadow`) but its unselected border is now **Ash** (changed from Mist), so the presets read as bordered pills. The panel is now an **anchored popover card** that overlays the schedule without pushing the day pager down (see SCHED-13); it closes on a re-tap of TIME, a tap on the schedule outside the card, or DONE. The removable chip carries a soft shadow and its × well presses at 90% scale.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`TimeFilterPanel`, `TimeRangeSlider`, `FilterPill`, `SelectablePill`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`setTimeFilter`, `clearTimeFilter`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/FilterChip.kt
- **Platforms:** shared
...
- **Expected:** Exactly one side is active at a time; switching resets the studio/location subset (`ScheduleFilters()`) while preserving the Time + Modality overlays, and triggers a debounced refetch scoped to the member's expanded favorite locations (Favorites) or the manual studio/location selection (All Studios). When the member has no favorites, only an "ALL STUDIOS" bar renders (no toggle). The sliding highlight now settles on the app's tuned `Springs.Settle` spec explicitly (was an untuned default spring) and carries its own `controlShadow`; the no-favorites "ALL STUDIOS" bar now presses (97% scale) with a `controlShadow` of its own. The selected side's fill is now **Moss** (changed from Ink): the sliding thumb and the no-favorites "ALL STUDIOS" bar are both Moss, with the on-fill label in Stone.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`useMyFavorites`, `showAllStudios`, `effectiveLocationIds`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`ScopeToggle`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/Motion.kt (`Springs.Settle`)
- **Platforms:** shared

### SCHED-12 — Favorites nudge banner (no favorites yet)
- **Steps:** Log in as `accounts.<device>.member_no_favorites` (seeded with zero favorites) and land on Schedule. Swipe to a different day while the banner is showing, then dismiss it from either day.
- **Expected:** A dismissable card banner — now `PaperTranslucent` with a `cardShadow` (decision H's token; opaque `Paper` if Cole rules the other way, in which case read "Paper-card" here) — reads "Make it yours. Save your favorite Studios." with a "CHOOSE FAVORITES" link that calls `onManageFavorites`; the ✕ dismisses it for the session only (`nudgeDismissed`, resets on process restart). "CHOOSE FAVORITES" now dims to 70% alpha on press (no scale, like `TextLink`, though it is its own implementation, not the shared component) and the ✕ well presses at 90% scale. The banner is now the header content of **every** day's page in the Schedule pager (`DayPage`'s `header` slot) rather than a single list item pinned once for the whole screen: it appears atop whichever day is on screen. The fade-up-and-rise entrance (`TransientSurface`, `collapse = true`) plays only on pages that were already composed when the favorites fetch resolved and the banner turned on: on a cold Book tab that is the landing day plus the two neighbour pages `beyondViewportPageCount = 1` keeps alive. Swiping to a day whose page has never been composed shows the banner already in place there, with no entrance — `AnimatedVisibility` seeds `currentState == targetState` on a fresh mount, so a banner that is already on cannot animate in. That is the intended reading, not a defect: the page itself is translating under the finger, and a second entrance riding a moving page would still be mid-flight when the page lands. Dismissing it from any one day hides it on every day for the rest of the session — `nudgeDismissed` is a single shared flag, not per-page. It drops back out the same way it came in (fades and collapses, never pops), and the list below closes the gap as it leaves rather than snapping up after it, with no gap left behind. The banner never shows if the favorites fetch failed (`favoritesKnown == false`), to avoid nudging a member who may already have favorites. **Silent-success note:** this suppression depends on `bodyOrThrow` (see FAV-05) — before it, a 5xx with a JSON error body on the member's first-ever favorites fetch silently reported an empty `FavoritesDto` as success, making `favoritesKnown == true` with zero favorites, which would have WRONGLY shown this nudge to a member whose real favorites status the server never actually confirmed.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`SuccessContent`, `FavoritesNudge`, `showNudge`, `nudgeDismissed`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`header` parameter, "page-header" item), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`hasFavorites`, `favoritesKnown`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/favorites/FavoritesRepository.kt (`refresh`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Transient.kt
- **Platforms:** shared

### SCHED-13 — Studio/location accordion filter (All Studios subset)
- **Steps:** With scope on "ALL STUDIOS", tap the bar to expand the accordion; tap a studio row to select/deselect the whole studio, or expand a studio's chevron and toggle individual locations. With enough studios expanded that the panel's content is taller than the screen, look for the DONE button.
- **Expected:** Selecting a whole studio clears any of its individually-picked locations (redundant); selecting every location under a studio individually promotes the pick to a whole-studio selection. Every change narrows the schedule server-side via the debounced pipeline. The controls (scope toggle, the Time / Modalities buttons and the active-filter chip rail) stay pinned above the day pager; opening a panel no longer pushes the pager down. Each of the four panels (Favorites / All Studios / Time / Modalities) now opens as an **anchored popover card** that OVERLAYS the schedule: it drops down directly under the controls (the existing `Dur.Medium` + `Ease.Emphasized` downward reveal, with a smooth reverse exit that mirrors it), floats above the day pager (which does **not** reflow), and is a plain rounded rectangle — `Surface` fill, `cardShadow`, rounded `ArcanaShapes.Card` corners on **every** side (no pointer/divot), a Mist hairline — inset horizontally to match the controls' content width so the card is **never wider than the controls**. Only one panel is open at a time; tapping the open control again, tapping the schedule outside the card, or tapping DONE closes it (DONE presses at 97% scale, fill darkens toward Moss, `controlShadow`). When a panel's content is taller than the cap (`FILTER_PANEL_MAX`, further capped so a slice of the schedule stays visible below it), the card scrolls internally with a soft bottom fade so a cut row peeks and "more below" reads clearly. In the All-Studios panel each studio is a new **compact single-line row** (`CompactStudioFilterRow`, the filter's own row): a small **square** check (Lime filled when the whole studio is chosen, a Lime ring + dot for a partial pick, a Mist outline otherwise), the studio name at the favorites-list text size with an inline "· N locations" (or "· N of M locations" when partial), and a chevron that rotates and expands that studio's `StudioLocationRow` location list below. The tall `StudioAccordionCard` is deliberately **no longer used in the filter** and is unchanged: it still renders on the favorites StudioSelection screen (see FAV-07) and the Design-system reference (DEVSET-12). This popover treatment is shared by all four panels; the Time panel now uses the card's standard 16dp horizontal inset like the others.
- **Expected (2026-09-15 addendum):** The All Studios card lists one row per brand from the overview's `brands[]` block (ID Hot Yoga once, expanding to its five locations; solidcore once including Jersey City), never a site row beside its brand; picking a brand sends every location behind it. A server without `brands[]` falls back to the `studios[]` block unchanged.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`toggleStudioWhole`, `toggleLocation`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`FilterControls`, `FilterPopoverOverlay`, `FloatingFilterPanel`, `CompactStudioFilterRow`, `filterPanelEnter`/`filterPanelExit`, `FilterDoneButton`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Depth.kt (`cardShadow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/StudioAccordion.kt (`StudioLocationRow`)
- **Platforms:** shared
...
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`TimeBand`, `timeBand()`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`byBand`, `activeBands`, `bandStart`, "band-header-$band" items, 24dp spacer)
- **Platforms:** shared

### SCHED-19 — Blocked/hidden session never surfaces and is not drillable
- **Steps:** Read the `classes.blocked` session id from the manifest (seeded `availability='blocked'` on Regression Test Studio / Regression Flatiron, **+2 days at 13:00 ET** — the same day as the `classes.full` fixture, which sits at 12:00 ET on the same studio). Select that day on the Schedule day rail with the seeded member's default Favorites scope, let page 1 settle, then scroll the whole day to the `EndOfListMarker` so pagination cannot be hiding it. Compare what rendered against the day's fixtures.
...
### SCHED-20 — Search entry point on the Book header
- **Steps:** Log in with `accounts.<device>.member`. On the Book tab, tap the search pill (magnifying-glass oval filling the space right of the month title; its "Search" label hides when a long month name leaves little room).
- **Expected:** A full-screen Search takeover opens via a container-transform: an oval grows out of the tapped pill (starting in the pill's Paper fill, settling to Stone) while the Book tab dims beneath it and the oval's rim carries a soft shadow, then the search content fades in. The keyboard does NOT open automatically (it appears when the member taps the field); the tab bar is hidden while it is open (NAV-04's assertion). The X control (top left, matching Class Detail) closes it with the mirrored animation and returns to the schedule exactly as it was left — same selected day, same scroll offset, no reload.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`onOpenSearch` header control), sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/navigation/ArcanaDestinations.kt
- **Platforms:** shared

### SCHED-21 — Unloaded day shows skeleton rows
- **Steps:** Swipe or tap to a day whose page 1 has not finished fetching under the current filter set (e.g. a day beyond the initially-loaded window, or any day right after a filter change forces a refetch).
- **Expected:** That day's page shows six shimmering skeleton placeholder rows (`SkeletonClassRow`, varied widths, one shared shimmer sweep per page) in place of the class list, instead of the old centered `DotMatrixLoader`. The pinned header above (month title, search pill, day rail, filter section) stays interactive throughout. Once the fetch resolves, the skeletons are replaced by the real rows, which then rise into place (SCHED-22); if the fetch instead fails, the page shows the inline error card with retry (ERR-03) — never skeletons.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`SKELETON_ROWS`, `SkeletonClassRow`, `skeletonBrush`, `dayLoaded`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Shimmer.kt (`ShimmerBox`, `shimmerBrush`)
- **Platforms:** shared

### SCHED-22 — Day rows rise into place once after a page settles
- **Steps:** Swipe to (or tap a chip for) a day whose sessions load or are already cached, and watch the rows appear. Then scroll that day's list past its first eight rows and back up.
- **Expected:** Once a page is current and its day is loaded, the first eight session rows fade in while rising 10dp into place with a 16ms stagger between each (band headers don't consume a stagger slot, only session rows do; a day's ninth-and-later rows appear immediately at full opacity with no rise). This plays once per page settle: the rise is driven by a single shared clock for the whole page, sampled per row, so scrolling a risen row off-screen and back does not replay it. Each row's capacity fill bar also animates in from empty to its value on the same page-settle event, via its own separate page-level clock (`capacityElapsed`, distinct from the rise clock since a still-filling bar should not visually "snap" the way a stagger can) — like the row rise, it does not replay on scroll-back within the same page, and a `ClassRow` outside a `DayPage` (Search's results) has no clock and renders its bar directly at its final fill, with no animate-in.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`STAGGER_ROWS`, `STAGGER_STEP_MS`, `RISE_DISTANCE`, `riseIn`, `staggerElapsed`, `capacityElapsed`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`ClassRow`, `barClock`)
- **Platforms:** shared

### SCHED-23 — "See schedule" from a studio page scopes the Book tab to that brand
- **Steps:** From Discover open a multi-location brand (locally `[solidcore]`), tap "See schedule". Then on the Book tab remove the brand chip; separately, tap a studio row in the All Studios popover while the chip is showing.
- **Expected:** The Book tab opens (Android: the tab bar's Book destination; iOS: the shell selects the Book tab and the studio page stays on the Discover tab's stack) with scope All Studios narrowed to exactly that brand's location ids (`ScheduleScopeRequests` hand-off consumed by `ScheduleViewModel.applyBrandScope`, no favorites change), a removable Moss chip carrying the brand name at the front of the chip rail, and the day list refetched server-side for those locations. Removing the chip restores the scope and picks that were active before the hand-off; when the hand-off lands before the Book tab has resolved favorites (first visit to Book in the session, iOS especially), the brand scope still wins and removing the chip restores Favorites. Any manual studio or location pick, or switching Favorites / All Studios, drops the chip without restoring. `schedule_filter_changed` fires with `mode=all` and the location count on apply and again on clear.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScopeRequests.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`applyBrandScope`, `clearBrandScope`, `dropBrandScope`, `applyFavoritesScope`), sharedLogic/src/commonTest/kotlin/org/arcana/mobile/schedule/ScheduleViewModelBrandScopeTest.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`FilterControls`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageViewModel.kt (`requestSchedule`)
- **Platforms:** shared

## CLASS

### CLASS-01 — Class detail loads from a schedule row tap, with a rise-and-recede push and a faster drop-back
- **Steps:** Tap any class row on Schedule, then close it with the header's close (X) control (or Android system back).
- **Expected:** Navigates to Class Detail; `ClassDetailViewModel.reload()` fetches `GET /api/v1/classes/<id>/` and fires the `classViewed` telemetry event once loaded (studio/location/modality/spots/full/load-ms). While loading, a centered `DotMatrixLoader` shows under the close button. The push rises from 12% of the screen height while fading in over 340ms (`Ease.Emphasized`), and the Schedule list beneath scales down to 96% on the same curve; there is no darkening of the list during this — the design spec describes a 12% ink dim, but it is not implemented, and this is a known, open item, not a defect to file. Tapping close reverses it, faster: over 200ms the returning list scales back to full size (`Ease.Emphasized`) while the leaving detail slides down 12% and fades out (`Ease.Exit`). The system back **gesture** now plays this same drop-back (`popExit`) as the X on both platforms: a `BackHandler` in each detail state routes back through the same `popBackStack`, so the iOS edge-swipe and Android predictive-back no longer run the platform's own interactive pop (slide-off-right / shrink-to-background). It is disabled while the booking or cancel sheet is up, so back dismisses the sheet first. (On the iOS 26 simulator the edge-swipe interactive preview differs from an iOS 18 device; the committed pop is the same push-down either way.) Tab-to-tab switching (NAV-01) is unaffected and stays instant. The header's close (X) well now presses (94% scale) and sits as `PaperTranslucent` with a soft shadow (decision H's token; opaque `Paper` if Cole rules the other way) instead of opaque `Paper` with no press feedback. The hero card at the top now carries a `cardShadow`, and the brand/location pill inside it now sits as `PaperTranslucent` with a soft shadow too (same token, same caveat).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt (`LoadingBlock`, `ErrorBlock`, `SuccessBlock` back handlers, `CircleIconButton`, `HeroCard`, `BrandLocationChip`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailViewModel.kt (`reload`, `fetch`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/NavTransitions.kt (`enter`, `exit`, `popEnter`, `popExit`)
- **Platforms:** shared
...
- **Expected:** Headline reads "FULLY BOOKED"; the segmented pip strip shows all pips as taken (Mist@70%) — the strip now fills in via an animated sweep (draw-phase only, no per-pip recomposition) from empty up to its taken count on load/refresh, rather than every pip snapping straight to its final state; the sticky CTA reads "CLASS FULL" in a Graphite pill with a clock icon instead of an arrow, and is disabled.
...
- **Expected:** The sticky CTA presses on the way down (scale-down with a fill that darkens a step toward Ink, a `controlShadow` while enabled — mirrors `PrimaryCta`) and its trailing arrow well kicks 2dp on release, the same kick `PrimaryCta`'s own well does; its label is now optically centred (`opticallyCentredCapsVertical` — the label is start-aligned, so only the vertical cap nudge applies). `BookingSheet` opens as an `ArcanaSheet`: a Paper surface on rounded top corners with a centered Mist handle, over a partly-transparent Ink scrim, and the class detail page behind it scales down to 94% and grows rounded corners of its own for as long as the sheet is open. It shows class name/studio, a credit-usage line ("This uses 1 of N credits"), and the late-cancel cutoff copy (bold-highlighted window from `bookingCancelCopy`). With the `BookingGestures.enabled` trial switch on (its default) and no accessibility service active, the sheet's commit control is a **slide-to-book** track (`SlideToConfirm`) rather than a CONFIRM tap: a Lime thumb the member drags right, an accent fill following it and the label fading, arming at 85% travel (the well turns Stone, a threshold haptic fires) and committing the reservation on release; light `Haptics.tick()`s fire at quarter-travel intervals building toward that arm point, and once the slide completes an indeterminate highlight sweeps the filled bar while the ~1-2s `createBooking` call is in flight (then the arrow→check confirm of CLASS-27 lands). Releasing before 85% springs home on `Springs.Settle`. With the switch off, or when TalkBack/VoiceOver is active (`useBookingGestures()` returns false), it is the plain "CONFIRM" `PrimaryCta` tap instead. Either control calls the SAME `createBooking` via `confirmBooking`, once and at the same moment — the ViewModel and request are unchanged. On success the sheet closes (after the in-place-confirmation hold, see CLASS-27), `bookingSucceeded` telemetry fires, the confirm haptic fires, and the CTA follows the status the server returned — "REQUESTED ✓" at a manual-fulfilment studio, "CONFIRMED ✓" at a direct-integration one (which confirms inside the create call).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/BookingSheet.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/BookingViewModel.kt (`openSheet`, `confirmBooking`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/BookingEligibility.kt (`bookCtaState`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ArcanaSheet.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt (`StickyReserveCta`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/SlideToConfirm.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/BookingGestures.kt (`useBookingGestures`)
- **Platforms:** shared
...
- **Steps:** Use the `classes.late_cancel_active` session from the manifest (the same fixture CLASS-20 uses), booked and then fulfilled per the runbook's Phase 3 fulfilment step so the member holds a live `confirmed` booking on it. Tap the sticky CTA to open the cancel sheet, then tap "CANCEL RESERVATION". (The seeded per-device reserved booking is the fallback fixture if that session is unavailable, but it is outside its cutoff and so exercises only the refund branch.)
- **Expected:** `CancelReservationSheet` (shared with the Reservations list) presents the same way `BookingSheet` does — an `ArcanaSheet` (Paper, rounded top corners, a centered Mist handle, a partly-transparent Ink scrim) with the class detail page behind it receding to 94% and gaining rounded corners while it's open — showing the class name, spot label (if any), and a forfeit/refund line driven by `cancelPolicy.willForfeitCredit`. With the `BookingGestures.enabled` switch on and no accessibility service active, the commit control is **hold-to-cancel** (`HoldToConfirm`): a Clay well whose ring fills over 700 ms with a rising haptic; releasing early cancels the gesture with nothing committed, holding through to a full ring fires the reject haptic and commits. With the switch off, or under TalkBack/VoiceOver, it is the plain "CANCEL RESERVATION" `PrimaryCta` tap. Either control calls the SAME `cancelBooking` via `confirmCancel`, once and at the same moment — the ViewModel and request are unchanged. On success the sheet closes, `existingBooking` clears, the CTA reverts to bookable/full/etc, and `bookingCancelled` telemetry fires with `creditRefunded`/`lateCancel`; on the TAP path a reject haptic also fires on the cancel-success transition (the gesture path's reject already comes from `HoldToConfirm`, so it is not double-fired).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/CancelReservationSheet.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/BookingViewModel.kt (`openCancelSheet`, `confirmCancel`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ArcanaSheet.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/HoldToConfirm.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/BookingGestures.kt (`useBookingGestures`)
- **Platforms:** shared
...
### CLASS-22 — Reservations: Upcoming segment, day headers, cancel sheet, and studio-cancelled rows
- **Steps:** From Home tap "See all" (or from the You tab tap "Reservations"). The list should already hold the seeded confirmed reservation from `accounts.<device>.member`. Tap "Cancel" on an upcoming row — to see the forfeit copy, cancel the `classes.late_cancel_active` booking prepared for CLASS-20 (fulfilment step applied); to see the refund copy, cancel any other upcoming row. To see a studio-cancelled row, have the server mark one of the member's upcoming sessions cancelled by the studio (`cancelled_by = "studio"`) before opening the screen.
- **Expected:** The title reads "Reservations" and a two-segment control (UPCOMING | PAST) sits under it with Upcoming selected on open; switching segments fires the selection haptic and `reservations_segment_changed`. Upcoming is read from `GET bookings/me/?scope=upcoming` and grouped under day headers in the studio's clock: "Today · Wed Sep 9", "Tomorrow · Thu Sep 10", then "Sat Sep 12" (`groupReservationsByDay`). Each row keeps the class name, "Tue, Jun 2 · 5:00 PM", studio · location · spot · with instructor, the existing `StatusPill`, and a "Cancel" link on `requested`/`confirmed` rows only; tapping the text block opens class detail. A studio-cancelled row stays in Upcoming until its start time with the "Cancelled" pill and no Cancel link. "Cancel" opens the shared `CancelReservationSheet` ("Cancel reservation?", forfeit or refund line from `cancelPolicy.willForfeitCredit`, hold-to-cancel or "CANCEL RESERVATION" per CLASS-18); confirming drops the row immediately, fires `booking_cancel_started` then `booking_cancelled`, and refetches Upcoming. A failed cancel keeps the sheet open with the transport copy. A "Book a class" ghost pill closes the list at the end of Upcoming and switches to the Book tab. The system back gesture pops the screen with the same transition as the close X (`BackHandler`), except while the cancel sheet is open, when back closes the sheet.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/ReservationGrouping.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/CancelReservationSheet.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/SegmentedControl.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Buttons.kt (`GhostCta`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`navigateToTab`), sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt (`ReservationsRoot`), sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/IosShellBridge.kt (`requestTab`), iosApp/iosApp/ArcanaShell.swift (`selectTab`)
- **Platforms:** shared

### CLASS-23 — Reservations: shimmer, empty states, pull-to-refresh, and the paged Past segment
- **Steps:** Open Reservations (a) the instant it loads, (b) as a member with zero upcoming and zero past reservations, and (c) as a member with more than 20 past reservations, then tap PAST and scroll to the bottom. Separately pull down on either segment, once with the server reachable and once with it stopped (`tools/regression/error-state-harness.sh kill-server`).
- **Expected:** (a) Four shimmer rows render while a segment loads; no "Loading…" caption. (b) Empty Upcoming reads "Nothing reserved yet." over "Your week is open." with a "Book a class" ghost pill; empty Past reads "Your first class will show here." (c) Past is read from `GET bookings/me/?scope=past` 20 rows per page, newest first, with no day headers and no Cancel links; scrolling within 10 rows of the end loads the next page behind a single shimmer row (`reservations_page_loaded` with `page_index` and `count` each time), rows are deduplicated by id, and the last page stops requesting. A failed later page keeps the rows already shown and appends an `InlineError` whose retry re-requests that page; scrolling alone does not retry. Pull-to-refresh re-reads the visible segment (Past resets to page one and discards a stale in-flight page); with nothing loaded a failure shows `FullScreenError` (CONNECTION vs SERVER) centred on the WHOLE screen, at the same height as Home's, with the close X, title and UPCOMING / PAST pills drawn over it and still live (ERR-06); with content on screen it keeps the content and shows the `ErrorSnackbar` refresh-failed toast. Scrolled away from the top, Past shows the Book tab's jump chevron at the top centre of the list (`JumpToTop`); tapping it animates back to the first row and the chevron leaves.
- **Expected (2026-09-15 addendum, reviews; rewritten 2026-09-18):** Every past row whose `review_state` is `none` carries the same `ReviewCard` as Home in its `Inline` style (16dp padding, a 16sp question): "Would you take this again?" with Yes / Maybe / No as outlined pills, each led by its mark (green check, amber warning, red minus; REVIEW-01), ALREADY inside the tonal well before anything is tapped, so answering changes the question and never the container. One tap POSTs `bookings/<id>/review/` with `prompt_surface=past`; every OTHER row of the same combination (brand, class type, instructor) flips to a Moss "Reviewed" caption at once, while the answered row keeps asking inside the same well: "How hard was it?", then the three recommend rows and the comment, exactly as on Home. Done (or the Done link on the intensity step) closes the card and the row reads "Reviewed". A pull-to-refresh mid flow keeps the open card. Rows already reviewed (`reviewed_here` or `reviewed_elsewhere`) show "Reviewed" and nothing else; ineligible rows (an elapsed Requested, a no-show) show neither. A failed answer keeps the step, puts the pick back to the last saved one and raises the screen's save-failed notice (REVIEW-01); nothing the member typed is lost. Leaving the screen mid flow loses nothing: step one already saved the review, and the row reads "Reviewed" on return. Tapping the row still opens class detail, where the card continues in edit mode. The day-header rule on the Upcoming segment is MossLight.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsViewModel.kt (`reviewStarted`, `reviewFinished`, `openReviews`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCard.kt (`ReviewCardStyle.Inline`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`JumpToTop`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/networking/ArcanaApiClient.kt (`myUpcoming`, `myPast`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/BookingDto.kt (`MyUpcomingDto`, `MyPastDto`, `reviewState`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ChoicePills.kt
- **Platforms:** shared
...
### CLASS-27 — In-place booking confirmation (no overlay)
- **Steps:** With the `BookingGestures.enabled` switch on (default) and no accessibility service active, complete a booking from the sheet via the slide-to-book control (CLASS-08). Watch the slide well and the sticky CTA as the booking succeeds, then let the sheet close on its own.
- **Expected:** While the `createBooking` call is in flight the filled bar carries an indeterminate sweep (CLASS-08); on success the slide well morphs arrow → check with a spring pop (`Springs.Kick`) and the label reads "BOOKED" (or "REQUESTED" at a manual-fulfilment studio, keyed on `existingBooking.status == "requested"`); the confirm haptic fires; the sheet stays on screen for about half a second (`CONFIRM_HOLD_MS = 550`) in that completed state before it closes as it does today, and the page behind it stays receded for that half-second (`holdForConfirm`) rather than popping back to full size early. As the sheet closes, the sticky CTA shows its confirmed label ("CONFIRMED ✓" / "REQUESTED ✓") and its arrow well pulses Lime once (~540 ms, a single grow-and-fade). There is NO full-screen celebration, overlay, or takeover: confirmation is in place only. With the switch off or a screen reader active, the sheet closes immediately as before, `holdForConfirm` never arms, and only the confirm haptic and the CTA label change remain.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/SlideToConfirm.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt (`StickyReserveCta` pulse, `holdForConfirm`, the `LaunchedEffect(submit)` confirm haptic), sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/BookingSheet.kt (`booked`, `bookedStatus`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/BookingGestures.kt (`useBookingGestures`)
- **Platforms:** shared

### CLASS-28 — Tappable addresses: the maps sheet on class detail, reservation rows and the Home next-up card
- **Steps:** Open the class detail of a session whose location carries an address (any synced studio; the regression fixtures' `Regression Flatiron` carries "1 Regression Way, New York, NY 10001" with no coordinates). Tap the LOCATION row. Repeat from a Reservations row's address line and from the Home next-up card's address line. In the sheet tap each option once, then long-press a text field elsewhere to confirm the copied address.
- **Expected:** The location block is one control: Pin avatar, "LOCATION" overline, location name (studio name when the location has none), address and a trailing chevron, announced as "{name}, {address}, open in maps". Tapping it opens an `ArcanaSheet` titled with the business name ("Barry's Chelsea": studio plus location) over the address. iOS lists "Open in Apple Maps" always and "Open in Google Maps" only when the Google Maps app is installed (`canOpenURL(comgooglemaps://)`, whitelisted in Info.plist); Android lists "Open in Google Maps" only, which fires a `geo:` intent through the system chooser. Each maps option fires `address_tapped` with `surface` (`class_detail` | `reservation_row` | `home_next_up`) and `app` (`apple` | `google`), opens the app on a search for "{business}, {address}" (biased to the coordinates when present: Apple `sll`, Google `center`, Android `geo:lat,lng?q=`), so the studio's own listing comes up rather than a bare pin, and closes the sheet. "Copy address" fires `address_tapped` with `app=copy`, puts the address on the clipboard, flips its label to "Copied" with a check glyph and closes the sheet after about 0.7 s. Reservation rows and the next-up card show the address as a one-line `AddressLink` (address text plus a small chevron; Stone-toned on the Moss card) beneath the studio line, only when the session carries `location_address`; tapping the rest of the row or card still opens class detail.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/AddressRow.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/MapsLauncher.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/ui/MapsLauncher.ios.kt, sharedUI/src/androidMain/kotlin/org/arcana/mobile/ui/MapsLauncher.android.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/maps/MapLinks.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt (`LocationRow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsScreen.kt (`ReservationRow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/home/HomeScreen.kt (`NextUpCard`), iosApp/iosApp/Info.plist
- **Platforms:** shared

### CLASS-29 — Class detail of a completed reservation: the review card replaces the reserve control, the class type feedback link, the instructor sheet
- **Steps:** Open class detail for a class the member attended (a completed reservation, or a confirmed one whose end time has passed) with no review yet; answer the card through to Done, then reopen the class. Open a class of the same brand class type with reviews from other members (locally ID Hot Yoga after the local seed) and tap "See what members say about ID · N". Tap the instructor row.
- **Expected:** With `review_booking_id` set and `my_review` null, a `ReviewCard` (surface `detail`, the tonal well headed by a Moss "YOUR FEEDBACK" caps line, "Would you take this again?") sits in the list after the instructor row and the sticky reserve CTA is not rendered; with `my_review` set the card opens in edit mode showing every saved answer, each change PATCHing the carrying booking's `review/` (PATCH goes to `my_review.booking_id`, so a later class of the same combination edits the same review). On this surface the card NEVER collapses: Done presses in like every other pill (it springs back; it never sticks scaled down), fires the confirm haptic, and the same Moss pill settles as "FEEDBACK SAVED" with the arrow in its Lime well swapping to a check with a small pop (`SettlingCta`; the slide-to-book knob's bump). It keeps its colour, size and shadow, and only stops taking taps; any further change (a pill, a dot, an edited comment) wakes it back to DONE. It is never grey and never reads "Saving…". An already reviewed class opens rested. An upcoming or unbooked class shows the usual CTA and no card. Every way into a feed reads "What members say · N reviews" and sits directly under the thing it is about, only when that thing has reviews of its own: under the hero card when `class_type_review_count > 0` (feed scoped `class_type=<brand>:<key>`); under the instructor row when that instructor's `review_count > 0` (instructor scope); under the location row when `location_review_count > 0` (location scope), falling back to the brand's feed when the location has none but `brand_review_count > 0`. Each fires `feedback_feed_opened` with its `scope_type` and `source=class_detail`. No hint is shown for a subject with no reviews. The instructor row is tappable once it carries a `profile_id` (chevron shown): it opens the shared `InstructorSheet` with bio (or "No bio yet."), `instructor_tapped` fires with `source=class_detail`, and when the profile has reviews the same "What members say · N reviews" link opens the instructor-scoped feed. Back closes the sheet before it closes the page.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt (`SuccessBlock`, `InstructorRow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCard.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/InstructorSheet.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/ScheduleDto.kt (`myReview`, `reviewBookingId`, `classTypeReviewCount`, `locationReviewCount`, `brandReviewCount`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCopy.kt (`whatMembersSayLabel`), sharedLogic/src/commonTest/kotlin/org/arcana/mobile/review/ReviewViewModelTest.kt
- **Platforms:** shared
...
- **Expected:** Each favorited studio/location renders as a read-only row (a Moss dot + name + "All locations" or the specific location detail — never a tappable checkbox); a "MANAGE IN PROFILE" link calls `onManageFavoritesTapped()` telemetry then `onManageFavorites` navigation callback. `favoritesDropdownOpened` telemetry fires once when the panel is revealed. The panel is now an **anchored popover card** that overlays the schedule without pushing it down: it drops open directly under the controls with the shared downward reveal and its smooth reverse exit, is never wider than the controls, and closes on a re-tap of the FAVORITES segment, a tap on the schedule outside the card, or DONE (see SCHED-13 for the shared popover treatment).
- **Expected (2026-09-15 addendum):** With brand-grouped favorites the panel lists one line per brand: "All locations" when the saved rows cover every catalog location of the brand, otherwise "N of M locations"; location favorites follow as before.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`FilterControls`, `FilterPopoverOverlay` fav panel, `FloatingFilterPanel`, `FavoriteEntryRow`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`onFavoritesDropdownShown`, `onManageFavoritesTapped`, `favoriteEntries`)
- **Platforms:** shared
...
- **Expected:** A hero renders "Make it\nyours." (Display), "Save the places you keep coming back to." (body), and "Change anytime." (Moss accent). Tapping a chevron expands/collapses that studio's location list via `toggleExpanded`; the chevron itself now rotates smoothly instead of snapping, the card's fill/border cross-fade between chosen (Ink) and unchosen (`PaperTranslucent`/Mist — decision H's token; opaque `Paper` if Cole rules the other way) with a `cardShadow`, and the whole card presses at 99% (its check well at 90%).
- **Expected (2026-09-15 addendum):** Studio Selection shows one card per brand (site rows folded under the brand's name with all their locations, sorted A to Z), built by matching `GET /studios/` rows onto the overview's `brands[]` by `studio_id`. A row the overview cannot place (no sessions in the 15-day window) stays its own card; an overview failure lists every row ungrouped rather than failing the screen.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/studios/StudioSelectionScreen.kt (lines 163-170, hero; `toggleExpanded` panel), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/studios/StudioSelectionViewModel.kt (`toggleExpanded`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/StudioAccordion.kt (`StudioAccordionCard`)
- **Platforms:** shared
...
- **Expected (2026-09-15 addendum):** Toggling a brand card selects every location behind it; completing a brand's locations one by one promotes to the whole brand exactly as it did for a studio.
...
- **Expected (2026-09-15 addendum):** Save sends whole brand cards as `brand_slugs` and ungrouped row cards as `studio_slugs` (never a brand's site slugs); `favorite_added` / `favorite_removed` carry `type=brand` for brand cards and `favorites_saved` gains `brand_count` and `brand_slugs` alongside the row-grain counts. Reopening the screen restores a brand favorite that covers all its locations as a whole card, and a partial one as location picks.
...
- **Expected:** Renders `LoadErrorState`, which is the shared `FullScreenError` keyed to `ErrorType` plus an overlaid Close (X) — so the copy is the same as CLASS-02/PROFILE-05 ("SOMETHING'S OFF ON OUR END." / "Give it a moment and try again." on the SERVER leg, "CAN'T REACH ARCANA." / "Check your connection and try again." on CONNECTION), with a TRY AGAIN CTA that re-invokes `viewModel::load`. There is no screen-specific headline. The X (content description "Close without saving") bails out without saving.
- _Corrected after run 2026-09-04: the entry asserted a distinct "Couldn't load." headline; `profile/EditProfileScreen.kt`'s `LoadErrorState` renders the shared component verbatim and always has._ See ERR-16 for the silent-success history: before `bodyOrThrow`, a 5xx with a JSON error body reached a blank editable form instead of this error state.
...
### PROFILE-27 — "Reservations" row opens the Reservations screen
- **Steps:** From the You tab, scroll to the account rows and tap "Reservations" (between "Membership" and "Concierge").
- **Expected:** The row carries a calendar glyph and a trailing caret; tapping it pushes the Reservations screen (`MyBookings(source = "you")`, so `reservations_opened` carries `source=you`) inside the You tab: on Android via the single NavHost, on iOS inside the Profile tab's own back stack, with the native tab bar hidden while pushed. Rows on that screen open class detail within the same stack, and the close X returns to the You tab.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/profile/ProfileScreen.kt (`accountItems`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt (`ProfileTabViewController`)
- **Platforms:** shared
...
- **Expected:** Header reads "Reach the founders" with intro body copy, followed by a "Your message" field that behaves exactly like a review's comment: ONE line tall with its placeholder ("What's going on?", 14sp, a step under the 18sp the member types in) sitting on the rule, growing a line at a time to three as the member types, then scrolling inside those three lines. The Send CTA is a `SettlingCta` and is never grey: it is full Moss with its Lime arrow well whether or not anything is written. Tapped with a blank message it sends nothing and puts the cursor in the field; with a message it dismisses the keyboard and submits (`canSubmit` still guards the request).
...
### DEVSET-12 — Design system reference page
- **Steps:** Signed out on the Auth screen, tap the wordmark 10 times to open Developer Settings, scroll to the "Reference" section at the bottom, and tap "Design system". Scroll the page top to bottom, tap one motion track, tap OPEN A SHEET and dismiss the sheet it opens, tap the outlined and filled Tactile pills and the `StudioAccordionCard` sample, drag the slide-to-book thumb to its arm point and release, press and hold the hold-to-cancel well until its ring fills, and tap one haptics pill, then tap the header's back (X) circle. On Android, repeat and dismiss with system back instead.
- **Expected:** The settings content is replaced in place by a Stone page titled "DESIGN SYSTEM" with the overline "PHASE 0 · FOUNDATIONS" (still accurate: this header was deliberately left untouched by later phases), then these sections in order, each under a Moss `SectionRule`: Colours (every `AppColors.kt` token as a swatch plus name plus derived hex, grouped brand primaries / derived greens / stone tones / ink and structure / warm neutrals / functional / translucent helpers), Type (one sample per `ui/Text.kt` style), Shapes (five `ArcanaShapes` tiles), Depth (the four shadow modifiers plus a Moss tile with `controlShadow` + `innerHighlight`), Atmosphere (a 200dp card showing the live drifting surface, its recipe caption, and a status caption reading "Live mesh · drifting", or "Still · ..." under Reduce Motion / Low Power Mode / animations off; the page itself also sits on the atmosphere), Controls (enabled, disabled and Clay-variant `PrimaryCta`, filled and outlined `IconCircle`, `TextLink`, two `FilterChip`s), **Tactile** (decision H's comparison, live on the page's own drifting atmosphere: three same-size cards captioned unambiguously — opaque `Paper` with a `cardShadow`, `PaperTranslucent` with the same `cardShadow`, and `PaperTranslucent` with no shadow at all (the middle card carries a faint Ink veil inside its translucent fill because Compose's drop shadow draws a filled silhouette behind the content, not a ring; the third card isolates the fill alpha alone) — then a `PaperTranslucent` pill beside an opaque `Paper` pill, both captioned the same way; a one-line caption stating that every card, pill and sheet in the app now ships in `PaperTranslucent` and that this is the call Cole is being asked to make; below that, an outlined and a filled `FilterPill`-style pill sample; and a live `StudioAccordionCard` sample (hoisted `remember` state) whose chevron rotates and whose fill/border cross-fade between Ink and `PaperTranslucent` on tap), Gestures (a demo-only caption stating the slide never books here because there is no ViewModel; a "Slide to book" group label over a live `SlideToConfirm` track that arms at 85% travel and flashes its completed check for a beat before resetting; and a "Hold to cancel" group label over a live `HoldToConfirm` well whose ring fills on a press-and-hold and flashes "CANCELLED" briefly on completion — neither books nor cancels anything, so this is the only place to drive the two controls without a signed-in, bookable account), Motion (five tappable tracks whose Lime dot travels end to end on the named spec), **Navigation** (a "SHEET" group label over an OPEN A SHEET button, then a "PUSH AND POP" group label over three captions naming the push motion, the pop motion, and stating plainly that the screen beneath only scales because the spec's ink dim is not implemented; tapping the button opens a demo `ArcanaSheet` — Paper on rounded top corners with a centered Mist handle over an Ink scrim, reading "A sheet, opened" — and while it is open the whole page behind it, atmosphere included, scales down with visibly rounded corners against a flat band of Stone on all four sides, the sheet itself staying full width; the scrim dismisses it and returns the page to full size with no residual scale or tint, while Android's system back key exits the whole Design system page rather than just the sheet. This page is reachable with no sign-in on both platforms, which makes it the only place to see real sheet chrome without a signed-in, bookable account), Schedule (a day rail bound to a 7-day `HorizontalPager`, exactly like the Book tab's own day-rail-and-pager binding, plus two shimmering `SkeletonClassRow` placeholders below it), Tab bar (a live `ArcanaTabBar` whose dot travels between Home / Book / You), Haptics (nine pills firing the `Haptics` verbs; no perceptible feedback on a simulator or emulator) and Loaders. Tapping a motion track or a haptics pill animates/fires without navigating or crashing. Back (X) and Android system back both return to Developer Settings with the base-URL controls unchanged. The page has no entry point outside this gesture chain.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/settings/DesignSystemScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/settings/DeveloperSettingsScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ArcanaSheet.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/StudioAccordion.kt (`StudioAccordionCard`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Depth.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/AppColors.kt (`PaperTranslucent`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/SlideToConfirm.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/HoldToConfirm.kt
- **Platforms:** shared
...
- **Expected:** Tapping a tab swaps the screen instantly. On Android the bar itself is a floating Paper pill over the atmosphere, not a docked strip (see PLAT-09) — switching tabs works the same way regardless. The tapped item's icon and label highlight in Moss (the Lime indicator dot travels to it on a spring on Android; system selection styling on iOS), and the icon bounces (Android). On both platforms a selection haptic fires on the tap, but only when the tapped tab differs from the already-active one; re-tapping the already-active tab fires none. On Android it is `TabBar.kt` `onClick` → `Haptics.selection()`; on iOS the native `TabView` never calls that Kotlin path, so the shell fires a `UISelectionFeedbackGenerator` on a real switch (`ArcanaShell.swift` `tabSelected`, guarded on `previous != tab`). A haptic leaves no trace in a layout dump or screenshot (and neither platform's fires on a simulator/emulator), so score it by code read. Each tab's own scroll position/back stack is preserved when returning to it later in the session. The middle tab reads **BOOK** (renamed from SCHEDULE 2026-08-25). Only the two label strings changed: `ArcanaTab.Schedule`'s label and the Swift `Tab(...)` title. The enum constant, the `ArcanaDestination.Schedule` route and every telemetry name are deliberately still `Schedule`/`schedule` (see TEL-14) — do not "fix" that mismatch, it is what keeps the event stream continuous across the rename.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/TabBar.kt (`TravellingDot`, `TabItem`'s bounce), sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`navigateToTab`, `popUpTo(...){saveState=true}`/`restoreState`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/NavTransitions.kt (`tabToTab`, `isTabRoot`), sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt (per-tab `ComposeUIViewController`s), iosApp/iosApp/ArcanaShell.swift (`TabView`, `tabSelected` selection haptic)
- **Expected (2026-09-09 addendum):** The bar now carries four items, Home · Book · Discover · You, and the dot travels across four positions.
- **Platforms:** shared
...
- **Steps:** From any tab root, navigate into a non-tab screen (ClassDetail, StudioSelection, MyBookings, EditProfile, ConciergeRequest, Search, StudioPage, FeedbackFeed).
- **Expected:** The bottom tab bar (a floating Paper pill on Android; the native bar on iOS) disappears while the pushed screen is on top, so a stray tab tap can't silently pop the in-progress flow off the stack. On Android this is now a crossfade rather than an instant cut: the pill fades out over `Dur.Quick` (120ms) as the push begins and fades back in over the same duration on a pop, holding the last active tab through the fade-out so it never renders blank mid-transition. Returning to the tab root (back/close) brings the tab bar back.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`selectedTab` derived from `currentDestination?.hasRoute<...>()`, `AnimatedVisibility(visible = selectedTab != null)`, `lastTab`), sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt (`atRoot`, `onRootChanged`), iosApp/iosApp/ArcanaShell.swift (`.toolbar(shell.tabBarHidden ? .hidden : .visible, for: .tabBar)`)
- **Expected (2026-09-09 addendum):** `StudioPage` (Discover) joins the pushed, bar-hiding destinations; on iOS it lives on the Discover tab's own stack.
- **Platforms:** shared
...
- **Expected:** The tab renders the shared `FullScreenError`, keyed to `ErrorType`, with a small color dot beside the overline (Lime for CONNECTION, Burnt Nectar for SERVER). CONNECTION shows overline "Connection", headline "CAN'T REACH ARCANA.", body "Check your connection and try again."; SERVER shows overline "Server", headline "SOMETHING'S OFF ON OUR END.", body "Give it a moment and try again." A "TRY AGAIN" pill (Moss fill) calls `viewModel.reload()` and re-attempts the fetch; the pill (`RetryButton`) now presses like `PrimaryCta` — scales to 97%, its fill darkens a step toward Ink, a `controlShadow`, no ripple. No `"server error"` string literal remains anywhere in the app.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`applyRefetchFailure`, `reload`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`FullScreenError`, `ErrorCopy`, `RetryButton`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Pressable.kt (`pressable`)
- **Platforms:** shared

### ERR-02 — Schedule refetch failure with content already on screen keeps content, no error takeover
- **Steps:** From a loaded Schedule (Success state with visible classes), trigger a filter/day refetch (change filter, tap a day chip) and have that request fail. Separately, pull-to-refresh (SCHED-07) and have that request fail.
- **Expected:** Unchanged member-visible behavior for a filter/day change: `applyRefetchFailure(type: ErrorType)` (now `ErrorType`-typed, was a raw string) does NOT replace the screen with an Error block when `_uiState` is already `Success` — it just clears the `refreshingFilters` dim and re-publishes the existing list; the member keeps seeing their last-good schedule with no error banner or toast for that case. A pull-to-refresh failure is the one `source` value `applyRefetchFailure` treats differently: it additionally sets `_refreshFailed`, which raises the same shared `ErrorSnackbar` Home/Profile/Class Detail use ("Couldn't refresh", a Lime Retry, a dismiss X), fading up and rising into place (`TransientSurface`) rather than appearing instantly, and dropping back out the same way on dismiss or on any later successful fetch. Only a cold-start/error-retry failure (no content at all) produces the full-screen `Error` state. **Silent-success note:** this guarantee now depends on `bodyOrThrow`. Before it, a 5xx with a JSON error body did not throw at all here — `fetchOverview`/`fetchSessionsPage` silently returned an empty-but-valid DTO, and the atomic-apply step would have written that emptiness over the member's real schedule instead of preserving it. `bodyOrThrow` is what makes "keeps content" true for every 5xx shape, not just network-level failures.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`applyRefetchFailure`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/networking/ArcanaApiClient.kt (`bodyOrThrow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`ErrorSnackbar`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Transient.kt
- **Platforms:** shared

### ERR-03 — Schedule single-day page fetch failure renders an inline error card with retry
- **Steps:** Tap a day chip (or swipe to a day) whose page-1 fetch is not yet cached, and have that specific day's `fetchSessionsPage` call fail while other days/filters are unaffected.
- **Expected:** **Behavior inverted from before this branch.** The day area no longer sits silently on `DotMatrixLoader` forever, and it never falls back to the unloaded-day skeleton rows (SCHED-21) either — a fetch failure and an in-flight fetch are rendered differently. `ensureSelectedDayLoaded`'s catch block sets `Success.dayError` to the failure's `ErrorType`, and that day's page renders an `InlineError` card with an underlined "Retry" wired to `retryDay()`, with the pinned header (month title, search pill, day rail, filter section) staying live above it regardless of which day's page is showing the error. The card itself now sits as `PaperTranslucent` (decision H's token; opaque `Paper` if Cole rules the other way) with a `cardShadow`, rather than opaque `Paper` with no shadow. `dayError` clears on a real day switch, a filter change, and any refetch that actually (re)loads the day, so it can never outlive the failure it describes. **Silent-success note:** before `bodyOrThrow`, a 5xx with a JSON error body did not even reach this catch block — `fetchSessionsPage` silently returned an empty-but-valid `SchedulePageDto`, so the day was marked loaded with zero sessions and the list showed the unrelated "No classes match your filters for this day." message (SCHED-06) instead of either the old stuck-loading placeholder or today's `InlineError` — a server outage looked identical to a genuinely empty day.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleViewModel.kt (`ensureSelectedDayLoaded`, `retryDay`, `dayError`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`SuccessContent`, per-page `dayError` gating), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/DayPage.kt (`dayError` parameter, "day-error" item), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`InlineError`)
- **Platforms:** shared
...
- **Expected:** Cold-load failure (no prior content) renders the shared `FullScreenError` keyed to `ErrorType` (same CONNECTION/SERVER copy as ERR-01) with "TRY AGAIN" wired to `HomeViewModel.retry()` (same `RetryButton` press feedback as ERR-01 — 97% scale, darkened fill, `controlShadow`, no ripple), replacing the whole list rather than appearing inside it. A refresh failure while Success content is already on screen leaves that content untouched and instead raises a dark-Ink `ErrorSnackbar` reading "Couldn't refresh" (`ErrorCopy.REFRESH_FAILED`, one line, no second sentence) with its own Lime "Retry" (dismisses it and re-runs `refresh()`) and a small `X` dismiss control (`HomeViewModel.dismissRefreshFailed()`, clears it without retrying). The snackbar fades up and rises into place rather than appearing instantly, and drops back out the same way when dismissed, retried, or superseded by a later successful fetch (`TransientSurface`). Per a code comment (not device-confirmed), tab re-entry is not a reliable recovery path on either platform — iOS: an in-tab push/pop re-fires the load, a bare tab switch does not; Android: a tab switch does, via `NavHost`'s `popUpTo`/`restoreState` — which is why `retry()` exists as an explicit, always-reliable control instead.
- _Corrected after run 2026-09-04: the snackbar reads "Couldn't refresh" and nothing more (`ui/ErrorState.kt`, `ErrorCopy.REFRESH_FAILED`); the "Showing your last update." sentence this entry used to assert has never existed in the code._
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/home/HomeViewModel.kt (`fetch`, `retry`, `refreshFailed`, `dismissRefreshFailed`, `load`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/home/HomeScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`FullScreenError`, `ErrorSnackbar`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Transient.kt
- **Platforms:** shared

### ERR-06 — Reservations load failure shows the full-screen error, centred on the whole screen with the header still live
- **Steps:** Open Reservations (from Home's "See all") when the visible segment's first fetch fails (`tools/regression/error-state-harness.sh kill-server`, or airplane mode). Screenshot it and Home's cold-load error and compare the height of the "TRY AGAIN" button. Tap PAST, then the close X.
- **Expected:** The segment's `Error` state renders the shared `FullScreenError` keyed to `ErrorType` (copy at ERR-01) as a layer that fills the WHOLE screen, with the close X, the "RESERVATIONS" title and the UPCOMING / PAST pills drawn over it. The error block therefore sits at the same height as on Home, Book, Discover and You (its button measured 518 to 574pt on a 874pt iPhone on every one of them); it is NOT centred in the space left under the header, which is what put it visibly low. The header reports its bottom edge (`Modifier.chromeBottom`) and the error takes it as `topInset`, so on a short screen or at a large font size the block starts 24dp under the pills instead of sliding beneath them, and scrolls if it still does not fit. The X and both pills stay tappable over the layer, and "TRY AGAIN" re-requests the visible segment with the button carrying the progress (`retrying`). There is no inline card under the header any more.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsViewModel.kt (`retry`, `retrying`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`FullScreenError`, `chromeBottom`)
- **Platforms:** shared
...
- **Expected:** `CancelState.Failed(code)` — `code` is `"connection_failed"` or `"server_failed"` per `toErrorType()` (`"cancel_failed"` is a back-compat copy branch with no current producer) — keeps the cancel sheet open with the CANCEL BOOKING button re-enabled (no longer showing the "CANCELLING…" spinner state) and adds a burnt-nectar `Caption` via `cancelErrorCopy(code)`. For both live codes that copy is the **shared transport copy**, not a cancel-specific string: `bookingErrorCopy`'s `CONNECTION_FAILED, SERVER_FAILED` branch delegates to `transportErrorCopy(code)`, so the member reads "Something went wrong on our end. Try again in a moment." (server) or "Couldn't reach Arcana. Check your connection and try again." (connection). "Couldn't cancel. Try again." is reachable only via the producerless `cancel_failed` code or as the unmapped-code fallback. There is no distinct RETRY control — tapping CANCEL BOOKING again is the retry path.
- _Corrected after run 2026-09-04: the entry asserted "Couldn't cancel. Try again." for both live codes, but `booking/BookingCopy.kt` routes them through `transportErrorCopy` before the cancel-specific fallback can apply. The observed copy is correct and on-brand; the entry was describing a branch with no producer._
...
- **Expected:** `StudioSelectionUiState.Error` carries an `ErrorType` and renders the shared `FullScreenError`, so a 5xx reads "Something's off on our end." and a dropped connection reads "Can't reach Arcana." — they are no longer the same fixed "Couldn't load Studios." line. RETRY is wired to the VM's `retry()`, which **does not drop back to Loading**: the error stays on screen and the retry control carries the progress (`retrying`). The sticky Save bar is absent in this state (it renders only for `Ready`), so nothing can cover the retry. The error (and the loader) fill the whole screen with the close X drawn over them, so the block sits at the same height as Home's rather than in the space under the X (ERR-06).
...
- **Expected:** The existing content stays on screen unchanged (no error takeover, no shimmer). A dark-Ink `ErrorSnackbar` fades up and rises into place (`TransientSurface`) pinned to the bottom, reading "Couldn't refresh" (one line, no second sentence — see the correction below) with a Lime "Retry" link and a small `X` dismiss control (content description "Dismiss notice"). Tapping the dismiss `X` (`HomeViewModel.dismissRefreshFailed()`) drops the toast back out the same way, without retrying and without disturbing the Success content. Tapping "Retry" drops the toast out and immediately calls `refresh()` again; once the server is reachable this both clears the toast and updates the content. The toast never latches — any later successful `fetch()` (pull-to-refresh or otherwise) also clears it on its own.
- _Corrected 2026-09-06: this entry quoted "Couldn't refresh. Showing your last update." — `ErrorCopy.REFRESH_FAILED` is "Couldn't refresh" and nothing more, as ERR-05's own 2026-09-04 correction already established; this entry just hadn't been fixed to match._
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/home/HomeViewModel.kt (`refreshFailed`, `dismissRefreshFailed`, `fetch`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/home/HomeScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`ErrorSnackbar`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Transient.kt
- **Platforms:** shared
...
### TEL-24 — Search taxonomy: opened, performed, scoped, result tapped
- **Steps:** On a Debug build (telemetry echo on), open Search from the Book header, type `regression` and pause, tap a studio scope chip, then tap the first result row.
- **Expected:** The echo shows, in order: `search_opened` (once per Search entry), `$screen: Search`, `search_performed` with `query`, `result_count`, `scoped=false`, `search_scoped` with `entity_type=studio`, a second `search_performed` with `scoped=true`, and `search_result_tapped` with `position` and `scoped=true`. Typing under 2 characters emits no `search_performed`.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/analytics/Telemetry.kt (`searchOpened`, `searchPerformed`, `searchScoped`, `searchResultTapped`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/search/SearchViewModel.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`currentScreenName`)
- **Platforms:** shared

### TEL-25 — Reservations taxonomy: opened, segment changed, page loaded
- **Steps:** On a Debug build (telemetry echo on), open Reservations from Home, tap PAST, scroll to the end of a member with more than 20 past reservations, then close and reopen it from the You tab.
- **Expected:** The echo shows `$screen: MyBookings` (the screen name is unchanged for dashboards) and `reservations_opened` with `source=home` exactly once per open, `reservations_segment_changed` with `segment=past` on the tap (nothing on a re-tap of the selected segment), `reservations_page_loaded` with `page_index=0` and `count` when Past first loads and `page_index=1` when the next page lands, and on the second open `reservations_opened` with `source=you`. Cancelling from the list reuses `booking_cancel_started`, `booking_cancelled` (with `location_id`) and `booking_cancel_failed`; no reservations-specific cancel event exists. Choosing an option in the address sheet (CLASS-28) emits `address_tapped` with `surface` and `app`; opening the sheet alone emits nothing.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/analytics/Telemetry.kt (`reservationsOpened`, `reservationsSegmentChanged`, `reservationsPageLoaded`, `addressTapped`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/booking/MyBookingsViewModel.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/analytics/ReservationsTelemetryTest.kt
- **Platforms:** shared

### TEL-26 — Discover taxonomy: opened, filter changed, studio page viewed, schedule and favorite taps, instructor tapped
- **Steps:** On a Debug build (telemetry echo on), tap the Discover tab, pick one modality and one neighborhood, open a multi-location studio page, tap an instructor, tap "Add to favorites", then tap "See schedule".
- **Expected:** The echo shows `tab_tapped` with `tab=discover`, `$screen: Discover`, `discover_opened` exactly once per Discover ViewModel, `discover_filter_changed` with `category_count` and `neighborhood_count` on every toggle or chip removal, `$screen: StudioPage` and `studio_page_viewed` with `brand_slug` and `source=directory` once per page open, `instructor_tapped` with `profile_id` and `source=studio_page`, `studio_favorite_tapped` with `location_count` and `result=sheet` (or `added` for a one-location brand), and `studio_schedule_tapped` with `brand_slug` followed by the Book tab's `schedule_filter_changed`. The `api_request` endpoint buckets are `discover_directory` and `discover_studio`; the brand slug never appears in an endpoint name.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/analytics/Telemetry.kt (`discoverOpened`, `discoverFilterChanged`, `studioPageViewed`, `studioScheduleTapped`, `studioFavoriteTapped`, `instructorTapped`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/analytics/ApiRequestMetrics.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/analytics/DiscoverTelemetryTest.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`currentScreenName`), sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/IosShellBridge.kt (`tabScreenName`)
- **Platforms:** shared

### TEL-27 — Reviews taxonomy: prompt shown, step saved, dismissed, completed, edited, feed opened, item tapped
- **Steps:** On a Debug build, open Home with a review prompt and answer it through Done; on another prompt tap "Not now"; open a reviewed class's detail and change an answer; open the feed from Discover and tap an item.
- **Expected:** The echo shows `review_prompt_shown` (`surface` home | past | detail, `booking_id`) once per fresh card, however often a Reservations row scrolls out and back in (edit mode never fires it), `review_step_saved` after every landed request with `step` (again | intensity | recommend | comment), `again`, `intensity`, `scores_set` and `has_comment`, `review_completed` (`duration_ms`, `steps`) once on Done for a new review, `review_dismissed` (`surface=home`) on Not now, `review_edited` after each change in edit mode (never `review_completed` there), `feedback_feed_opened` (`scope_type`, `source`) once per feed, `feedback_item_tapped` (`scope_type`, `target=studio_page`), and `$screen: FeedbackFeed`. The `api_request` endpoint buckets are `review_create`, `review_update`, `review_dismiss` and `feedback_feed`; no booking id reaches an endpoint name.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/analytics/Telemetry.kt (`reviewPromptShown`, `reviewStepSaved`, `reviewDismissed`, `reviewCompleted`, `reviewEdited`, `feedbackFeedOpened`, `feedbackItemTapped`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/analytics/ApiRequestMetrics.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/analytics/ReviewTelemetryTest.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/analytics/ApiRequestMetricsTest.kt
- **Platforms:** shared
...
- **Expected:** The tab bar shows a solid **opaque Stone** background behind the items — not a transparent bar with items floating over content, and not the old blurred default material, which bloomed as a white halo over the Book tab's lime atmosphere. `ShellModel.init` pins both `standardAppearance` and `scrollEdgeAppearance` to `configureWithOpaqueBackground()` with `backgroundColor` = Stone (`#F5F2ED`) on iOS 18.x, working around UIKit's inability to detect Compose content as a scrolling `UIScrollView` while keeping the bar on-brand and halo-free.
- **Source:** iosApp/iosApp/ArcanaShell.swift (`ShellModel.init`, `#unavailable(iOS 26.0)`, `configureWithOpaqueBackground`)
...
- **Expected:** The last row/card scrolls fully clear of the floating tab bar rather than being obscured by it — each tab-root scrollable adds `LocalFloatingBarInset` as bottom content padding. Android now provides a non-zero value here too (see PLAT-09): both platforms feed the same composition local, each computing it at runtime from its own bar's real footprint.
...
### PLAT-09 — Android edge-to-edge with a floating tab bar
- **Steps:** On Android, scroll each tab root to its last item, and check the tab bar's bottom edge against the gesture-nav inset.
- **Expected:** The tab bar is a floating Paper pill over the atmosphere, not a docked opaque strip: a bordered, translucent stadium shape sitting above content with its own side and bottom insets, rather than reserving a slot for it. Screen content flows edge-to-edge underneath; each tab-root scrollable clears the pill by adding `floatingBarInset` (the device's own safe-drawing bottom inset, computed at runtime so it holds on any device, plus the pill's own gap and height) as bottom content padding, so the last row is never hidden under the pill and doesn't leave an oversized dead gap beneath it either.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`CompositionLocalProvider(LocalFloatingBarInset provides floatingBarInset)`, the `Box`/`NavHost`/`ArcanaTabBar` sibling layout), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/TabBar.kt (`floatingBarInset`, `ArcanaTabBar`, `barShadow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/FloatingBarInset.kt
- **Platforms:** Android-only
...
- **Expected:** Every tappable icon well hits to at least 48x48dp even where the circle is drawn smaller (`diameter` 32-40 at most call sites). Compose expands any pointer-input node to `ViewConfiguration.minimumTouchTargetSize`, whose interface default is 48dp and which neither the Android nor the skiko/UIKit implementation overrides, so this holds on both platforms. Measured on the Pixel_9_Pro AVD 2026-08-24: a `diameter = 36` well registered taps out to a 24dp radius (48dp target) and missed at 26dp. **`diameter` is a visual property, not a touch-target one** — do not raise it to improve reach, and do not file a bug from a screenshot showing a small circle. The expansion is distributed evenly around the layout and cannot overlap a neighbouring pointer node, so densely packed icons are the only case worth checking by hand. Every `IconCircle` with an `onClick` now presses too: it scales to 94% while held, no ripple; a FILLED well (`background` set) additionally carries a soft shadow. This is a change to the one shared component, so it is true at every call site above and everywhere else `IconCircle` appears — not something to re-verify screen by screen. (This is the press mechanic only: most `IconCircle` fills elsewhere in the app stayed opaque `Paper` — decision H's translucent token was applied only where its own named files call for it, not to this shared component's default background.)
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Buttons.kt (`IconCircle`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Pressable.kt (`pressable`)
- **Platforms:** shared
...
- **Expected:** Under 0.5pt on both axes. League Spartan's capitals sit ~0.0914em high in the em box, so a label centred by `CenterVertically` alone lands visibly high; `ui/OpticalCentering.kt` corrects it. **The two helpers are not interchangeable and the choice is not cosmetic.** `opticallyCentredCaps(fontSize, letterSpacingEm)` also nudges right, cancelling the trailing letter-space that a CENTRED label splits evenly — correct inside a filled control. `opticallyCentredCapsVertical(fontSize)` omits that, for a label sitting BESIDE a sibling glyph: a start-aligned label never splits the trailing space, so the horizontal nudge corrects nothing and instead shifts the label out of its gutter. Measured on the Pixel_9_Pro AVD 2026-08-25: `TextLink` was 4.0px (1.33pt) high at every call site and is 0.00pt after the vertical nudge, underlined and not; applying the full two-axis helper instead moved Home's "See all" from x=73 to x=76 against a 74px gutter it shares with the rows above it. Each component passes its type size to the helper from the same constant its `TextStyle` uses, so the two cannot drift. Four more labels adopted `opticallyCentredCaps` in the tactile pass, replacing an older `lineHeightStyle`-trim recipe that passed vertically but sat 0.67-1.17pt left (no compensating nudge for the trailing letter-space): the schedule scope toggle's label, the TIME/MODALITIES filter pill label, the filter DONE button, and the class-detail sticky CTA's primary label. Measured on the iPhone 17 Pro Max sim: three of four now pass cleanly on both axes; "TIME" improved from 1.17pt to 0.50pt left but sits exactly on this entry's own pass/fail boundary — a per-string side-bearing residual the shared formula does not fully close, flagged as a follow-up rather than chased with a per-control fudge.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/OpticalCentering.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Buttons.kt (`TextLink`, `PrimaryCta`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`RetryButton`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`ScopeLabel`, `FilterPill`, `FilterDoneButton`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ClassDetailScreen.kt (`StickyReserveCta`)
- **Platforms:** shared


### PLAT-12 — Atmosphere: the living surface under every Stone screen
- **Steps:** Signed out, look at the sign-in screen for ten seconds; then sign in and look at Home, Book, the You body below the Ink hero, a class detail, My bookings, Studio selection, Edit profile, Concierge and Search. Pull the You tab down past its top (iOS). Then turn on Reduce Motion (iOS) or set Developer options → Animator duration scale to off (Android), relaunch, and look again.
- **Expected:** Every Stone root shows the same lime-and-olive surface with a deep olive centre that drifts slowly (control points on 6 s and 7.5 s periods scaled 0.8 to 1.4, amplitude 15% of the screen, corners pinned, edge points sliding along their edge), with an olive `#A6B277` vignette (10%) that saturates at the corners. It fills the whole screen including the status-bar and horizontal safe areas (no white or flat-Stone bands at the edges), and sits under sheets, the You Ink hero, the class-detail CTA slab and the floating bar. On the You tab, pulling past the top reads as Ink, and the first items below the hero sit on the atmosphere, never on black. Under Reduce Motion, Low Power Mode or animations off, the same surface renders still at base positions. No screen shows flat Stone as its root except Developer settings. Idle on any of these screens costs about 0.6 ms of UI-thread draw per frame on Android and about 2% of a core on iOS, at 30 updates per second; scrolling and springs keep the display's native refresh rate.
- **Expected (2026-09-12 addendum):** Secondary text placed directly on the atmosphere (section rules such as MORNING / EVENING, class-row duration, "WITH" instructor and capacity labels including FULL, day and empty-state captions, address lines, Discover category and place lines, Profile footer and account labels, form labels and placeholders, signup and concierge body copy) renders in `Charcoal` (#3F4338, about 6:1 over the olive atmosphere), never `Ash` (#6B6E5F, about 3:1) or `Ash2`. `Ash` remains only on lifted surfaces (cards, popovers, sheets, the tab pill) and as hairlines/outlines; the full-class capacity bar is `Ash`, not `Ash2`. `Overline` and `Caption` default to `Charcoal`.
- **Expected (2026-09-12 addendum):** The field is always smooth: no sawtooth seams, hard ridges or sudden geometry anywhere on it, at any point in the drift. Those came from the mesh folding over itself at the prototype amplitude (0.15 of the width); the drift now runs at 0.08, which a simulation of 2,400 frames shows never folds, and `AtmosphereMeshFoldTest` locks it.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/Atmosphere.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/AtmosphereMesh.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/AtmosphereMath.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/AtmospherePlatform.kt, sharedUI/src/androidMain/kotlin/org/arcana/mobile/theme/AtmospherePlatform.android.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/theme/AtmospherePlatform.ios.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/profile/ProfileScreen.kt (`ProfileHero`)
- **Platforms:** shared
### PLAT-13 — iOS: the first tap on any text field does not hang
- **Steps:** iOS only. Cold launch the app (from the home screen, not from Xcode: a debugger multiplies this cost), go to You → Concierge and tap "Your message" as the first text field touched in that process. Watch the launch for a keyboard flash. For numbers: `sample <pid> 8 1 -file out.txt` around the tap and read the main thread's time under `-[UIApplication sendEvent:]`.
- **Expected:** The field takes focus at once (cursor, Moss rule) and the keyboard follows without the screen freezing. UIKit builds its text-input system the first time anything becomes first responder, synchronously inside that tap; `KeyboardPrewarm.run()` pays it once from `ShellModel.splashDidAppear()`, before the splash timer starts, by making an invisible `UITextField` first responder and resigning it in the same run-loop turn. That field carries an empty `inputView`, so UIKit is never asked to present the system keyboard: no keyboard, and no flash of one, can appear during launch, and the splash still runs its full minimum. The real keyboard appears normally on the first real tap. Measured on the iPhone 17 Pro simulator with the software keyboard visible (docs/perf/README.md, "First keyboard focus"): first tap 244ms blocked without the prewarm, 26 to 30ms with it; every later tap about 3ms either way.
- **Source:** iosApp/iosApp/ArcanaShell.swift (`KeyboardPrewarm`, `splashDidAppear`), docs/perf/README.md
- **Platforms:** iOS
### PLAT-14 — iOS: a malformed saved crash report cannot stop the app launching
- **Steps:** iOS only. Install the Debug build on the simulator, launch it once so the data container exists, then terminate it. Plant a corrupt SentryCrash report: `R="$(xcrun simctl get_app_container <udid> org.arcana.mobile data)/Library/Caches/SentryCrash/Arcana/Reports"; printf '{"user": 3}' > "$R/Arcana-report-00000000deadbeef.json"`. Launch the app three times in a row, waiting about eight seconds after each; after every launch check the process is alive (`pgrep -f "<udid>.*Arcana.app/Arcana$"`) and list `$R`. Then terminate, plant a genuine well-formed report in `$R` instead, and launch once more.
- **Expected:** The app reaches its normal first screen on all three launches and never dies during startup, and `~/Library/Logs/DiagnosticReports/Arcana-*.ips` gains no new file naming `SentryCrashReportConverter`. The planted corrupt file is gone after the first launch, and `$R` stays empty on the second and third. `TelemetryBootstrap.start()` runs `SentryReportPreflight.run()` immediately before `SentrySDK.start`, which deletes every `*.json` under `<Caches>/SentryCrash/<bundle name>/Reports` that is not a top-level JSON object whose `user`, `crash`, `report`, `system`, `process`, `debug` and `sentry_sdk_scope` are objects wherever present and whose `binary_images` is an array wherever present, plus anything unreadable or larger than 5 MB. A well-formed report is left in place and still uploads as a crash on that launch. Without the pre-flight, sentry-cocoa (9.17.1, and still 9.29.0 upstream) throws in `-[SentryCrashReportConverter initWithReport:inAppLogic:]` reading `report["user"]` with no type check, and the member cannot open the app again without reinstalling.
- **Source:** iosApp/iosApp/Analytics/SentryReportPreflight.swift, iosApp/iosApp/Analytics/TelemetryBootstrap.swift
- **Platforms:** iOS
...
### SEARCH-02 — Search-as-you-type: debounce, entity chips, chronological day-grouped results
- **Steps:** With the regression seed applied, in Search type `regression` and stop typing.
- **Expected:** One fetch runs after a ~250ms pause (not one per keystroke — the debug telemetry echo shows a single `search_performed`). Results list the seeded regression studios' upcoming classes in chronological order under day headers (e.g. `SAT AUG 30`), spanning the full bookable window rather than one selected day. A chip row above the list offers matched entities (e.g. "Regression Test Studio · Studio"); a neighborhood/address match offers a LOCATION chip ("<Studio> <Location> · Location") that scopes one site, distinct from the whole-brand Studio chip. Scrolling the results dismisses the keyboard, and the list resizes above it while it is up (recents included). Rows use the standard schedule row treatment, including its 0.99 press-scale on tap (the same shared `ClassRow`); a run of consecutive rows whose class NAME did not match the query carries a small "Matches …" caption naming what did on the run's FIRST row only (not echoed under every row). After the final page, the shared end-of-list marker (the caps line "THAT'S EVERY MATCH" alone, no dots above it) renders; while more pages exist, infinite scroll appends them instead. Typing a single character searches nothing and returns to the idle state.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/search/SearchScreen.kt (`ResultsContent`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/search/SearchViewModel.kt (debounce pipeline), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/search/SearchMatchReason.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/networking/SearchApi.kt
- **Platforms:** shared
...
### SEARCH-04 — No-results state
- **Steps:** Search a string matching nothing, e.g. `zzzzqqq`.
- **Expected:** A plain "No classes found for "zzzzqqq"." message with a one-line suggestion under it — no error styling, no retry control. Editing the query recovers normally. The copy contains no em or en dashes.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/search/SearchScreen.kt (`NoResultsContent`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/search/SearchViewModel.kt
- **Platforms:** shared

### SEARCH-05 — Search failure path keeps the query and retries
- **Steps:** Arm the fault injector with a 500 on the sessions path (recipes in docs/regression/driver-playbook.md), then search `regression`. Disarm and tap Try Again.
- **Expected:** An inline error with SERVER copy renders in the results area; the typed query stays in the input. Try Again after disarming loads the normal results for that same query. A connection-class failure (server down) shows CONNECTION copy instead — never "server error".
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/search/SearchScreen.kt (`InlineError` branch), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/search/SearchViewModel.kt (`retry`)
- **Platforms:** shared

## DISCOVER

### DISCOVER-01 — Discover tab renders the full A-to-Z studio list
- **Steps:** Signed in, tap the Discover tab (third of four: Home, Book, Discover, You). Wait for the list.
- **Expected:** Title "Discover" (no subheading), two pills MODALITIES and NEIGHBORHOODS, then one flat list of every active brand from `GET /api/v1/discover/studios/` sorted A to Z: a circle monogram tinted with the brand's `primary_color` (Moss when absent), the brand name, up to three category glyph-plus-name pairs, and one line "Flatiron, Tribeca · 2 locations" (a fourth-plus neighborhood folds into "+N"; one location reads "1 location"). Six shimmer rows show while loading; a cold-load failure shows `FullScreenError` (CONNECTION vs SERVER) with retry, centred on the whole screen with the "Discover" title drawn over it (the pills and list are absent), at the same height as Home's (ERR-06). Scrolled away from the top, the list shows the Book tab's jump chevron at its top centre (`JumpToTop`); tapping it animates back to the first row and the chevron leaves. Pull-to-refresh re-reads the list and a failed refresh keeps it with the refresh-failed toast. Category glyphs currently all render the default star glyph (`CategoryIcons` maps every slug to it until the ten stroke icons land).
- **Expected (2026-09-15 addendum):** Category glyphs are now per category: strength (dumbbell), sculpt (kettlebell), pilates, reformer, yoga (lotus), barre, HIIT & Bootcamp (bolt), cycle, dance (note), run, boxing (glove); any slug outside those eleven still renders the default star.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/DiscoverScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/discover/DiscoverViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/DiscoverDto.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`JumpToTop`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`FullScreenError`, `chromeBottom`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/CategoryIcons.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/StudioColor.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/TabBar.kt, iosApp/iosApp/ArcanaShell.swift
- **Platforms:** shared

### DISCOVER-02 — Modality and neighborhood filters reuse the Book tab's popover card
- **Steps:** On Discover tap MODALITIES, check "Yoga", tap DONE; tap NEIGHBORHOODS, check "Harlem"; tap outside the card; remove the "Yoga" chip; with a filter that matches nothing, read the list.
- **Expected:** Each pill opens the same floating card the Book tab uses (`FloatingFilterPanel`: elevated, rounded, its own scroll, DONE at the bottom, an outside tap closes it) anchored under the pills over the list; the catalogs come from the unfiltered directory so a narrowing never hides the other options. Every toggle dims the stale list, fires `discover_filter_changed`, and after a 250 ms debounce refetches server-side with repeated `category=` and `neighborhood=` params; picks render as removable `FilterChip`s under the pills (a chip's × un-picks it). No match reads "No studios match those filters." with a "Clear filters" link that empties both sets.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/DiscoverScreen.kt (`FilterControls`, `FilterPopover`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/FilterPopover.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/discover/DiscoverViewModel.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/discover/DiscoverViewModelTest.kt
- **Platforms:** shared

### DISCOVER-03 — Studio page sections
- **Steps:** From Discover tap a brand with a bio, amenities and several locations (locally `[solidcore]`; the regression brand `regression-multiloc` carries a bio, a cutoff and three locations). Scroll to the bottom.
- **Expected:** A pushed page (tab bar hidden) reached from `GET /api/v1/discover/studios/<slug>/`: close X; header with a rounded monogram tile, name, category glyphs and names, the neighborhoods and location count line, the tagline in the Cormorant accent when present, and "Add to favorites" (or a Moss "In your favorites" check once favorited); "About" with the bio (the server already falls back to the synced description); "Good to know" with the text, amenity pills and the cancellation window sentence built from `cancellation_cutoff_minutes` by `bookingCancelCopy`; "Classes" with each class type and its description clipped to two lines, tap to expand; "Instructors"; both sections preview their first five rows with a "Show all N classes" / "Show all N instructors" link that toggles to "Show fewer"; "Locations" as one `AddressRow` per location with the neighborhood as its overline (CLASS-28 sheet on tap); a sticky "See schedule" `PrimaryCta` floating over the list with no fade behind it. Sections with nothing to show are omitted. The system back gesture pops the page with the same transition as the close X (`BackHandler`), except while a sheet is open, when back closes the sheet. Loading shows shimmer blocks under the close X; a failure shows `FullScreenError` with the close X still reachable.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageScreen.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageViewModel.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/booking/LateCancelWindow.kt (`bookingCancelCopy`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt (`DiscoverTabViewController`)
- **Platforms:** shared

### DISCOVER-04 — Add to favorites: one tap for a single location, a picker sheet for several
- **Steps:** Open a one-location brand's page and tap "Add to favorites"; open a multi-location brand's page and tap it; in the sheet check two locations and tap Save; repeat with the server stopped.
- **Expected:** One location: the tap saves at once (toggle haptic), the label flips to "In your favorites" and Profile's "Your favorites" shows the location on next load. Several: an `ArcanaSheet` "Add to favorites" lists "All locations" plus one checkbox row per location (pre-checked for locations already favorited) in a list capped at half the sheet height that scrolls inside the sheet so Save stays on screen for brands with many locations (solidcore); the sheet itself never moves while that list scrolls or flings (its overscroll is swallowed), and only the handle, the scrim or back dismiss it. Save is disabled with nothing picked, and saving merges the picks into the member's current favorites and PUTs the replace-set at location grain (whole-studio favorites already held are preserved); the sheet closes when the brand shows as favorited. A failed save keeps the previous set and shows "Couldn't save your favorites. Try again." inline. An already favorited brand's control is inert.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageScreen.kt (`FavoriteLocationsSheet`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageViewModel.kt (`onFavoriteTapped`, `saveFavorites`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/favorites/FavoritesRepository.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/discover/StudioPageViewModelTest.kt
- **Platforms:** shared

### DISCOVER-05 — Instructor sheet
- **Steps:** On a studio page with instructors (a brand with sessions in the next 15 days whose instructors are linked to profiles), tap an instructor row.
- **Expected:** The shared `InstructorSheet` (an `ArcanaSheet`) with the instructor's monogram and name and their bio, or "No bio yet." when blank; `instructor_tapped` fires with `profile_id` and `source=studio_page`. When the profile has reviews (`review_count > 0`) a "What members say · N reviews" link (the same words as every other way into a feed) closes the sheet and opens the feed scoped to the instructor (`feedback_feed_opened` with `scope_type=instructor`, `source=instructor_sheet`); with none the sheet ends at the bio. No schedule link.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/InstructorSheet.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageScreen.kt (`InstructorRow`)
- **Platforms:** shared

### DISCOVER-06 — Tab bar and shells carry four tabs
- **Steps:** Signed in, tap each of Home, Book, Discover and You in turn on Android and on iOS; from a pushed studio page, background and foreground the app; sign out and back in.
- **Expected:** Android's floating pill shows four items with the travelling Lime dot landing on Discover (compass glyph) and `navigateToTab` keeping Discover's own back stack; iOS's native TabView shows a fourth "Discover" tab (SF Symbol `safari`, tab index 2, You moves to index 3) hosted by `DiscoverTabViewController` with its own NavHost; the bar stays visible on the Discover root (`TabRoot` counts Discover among the roots) and hides on the pushed studio page for that tab only. `tab_tapped` carries `tab=discover` and the bridge emits `$screen: Discover` on a real switch. Logout clears the Discover tab's ViewModelStore with the others and login rebuilds all four controllers.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/TabBar.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`navigateToTab`, `currentScreenName`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/NavTransitions.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt, sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/IosShellBridge.kt, iosApp/iosApp/ArcanaShell.swift, sharedUI/src/commonMain/composeResources/drawable/icon_compass.xml
- **Platforms:** shared

### DISCOVER-07 — Member feedback on Discover: the directory row and the studio page's "What members say" links
- **Steps:** With at least one review anywhere on the server (locally after the reviews seed), open Discover; tap the Member feedback card. Open a brand with reviews (ID Hot Yoga) and tap "What members say · N reviews" under the header; scroll to Locations and tap a location's "What members say" link. Open a brand with no reviews.
- **Expected:** When the directory's `feedback.review_count` is above zero a tonal-well card (the same recessed container as every review surface, no shadow, no Surface fill) headed "MEMBER FEEDBACK" in Moss with "N reviews" and a Moss arrow at the right leads the list, quoting the newest comment (two lines, ellipsised) with "<class type> at <brand>" under it; tapping opens the unscoped feed. With zero reviews no row renders. On the studio page `review_count > 0` adds a "What members say · N reviews" link between the header and About (no inline averages) opening the brand-scoped feed; each location with reviews carries its own "What members say" link under its `AddressRow` opening the location scope labelled "<Brand> · <Location>"; a brand with none shows neither. Every class type's and instructor's `review_count` arrives with the page for the sheet and class detail links.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/DiscoverScreen.kt (`MemberFeedbackRow`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/discover/StudioPageScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/TonalWell.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCopy.kt (`whatMembersSayLabel`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/discover/DiscoverViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/DiscoverDto.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/ReviewDto.kt (`DiscoverFeedbackDto`)
- **Platforms:** shared

## REVIEW

### REVIEW-01 — The review card: three steps, saving on every tap
- **Steps:** From Home (the prompt card), a past reservation row, or class detail of an attended class, answer "Would you take this again?"; then "How hard was it?"; then the three recommend rows and the comment; tap Done. Watch the network log or the Debug telemetry echo.
- **Expected:** Each again pill leads with a mark before its word: the green check emoji for Yes (the system's white check on a green square: Apple's on iOS, Noto's on Android), an amber warning triangle for Maybe, a white minus on a red disc for No (`AgainMark`, 16dp, decorative: the word is what a screen reader hears). The warning and the minus are DRAWN, identical on both platforms: Compose renders the warning-sign emoji as a bare black outline on iOS, so it must never be swapped for the emoji without checking an iPhone. All three keep their full colour on both the outlined and the picked (Moss) pill. The intensity scale is ONE to THREE and its three equal pills carry flames alone, the system fire emoji once, twice and three times (`IntensityFlames`, Apple's on iOS, Noto's on Android), no words; a screen reader hears Easy / Moderate / Hard. Both pill rows stand the same height, and the marks and flames sit centred in their pills (measured within 0.2dp). Yes / Maybe / No is the whole review: the tap POSTs `bookings/<id>/review/` with `again` and the surface (home | past | detail) and the card advances only once the answer lands. A save that does not land (no signal, a 5xx) keeps the step, fires the reject haptic and raises ONE notice at the bottom of the screen (`ReviewSaveNoticeHost`, an `ErrorSnackbar` with a dismiss control and no retry, gone after six seconds) in the shared submit-flow words: "Couldn't reach Arcana. Check your connection and try again." for a connection failure, "Something went wrong on our end. Try again in a moment." for a server one, never confused. There is no inline error under the pills any more. Nothing the member entered is lost: the comment stays in the field exactly as typed, Done stays live so the same tap retries, and it never reads "FEEDBACK SAVED" over words the server does not have. An unsent comment is also kept on the device (`ReviewDrafts`, written once typing pauses, on every failed save and when the screen closes; at most five, cleared at sign-out): close the screen, come back to that class later, and the words are in the field again with Done awake; the draft is dropped the moment the server has it. Every tap shows its answer at once and its request follows, one at a time and in tap order; nothing on the card dims, greys or relabels while a request is in flight (no "Saving…", no flash), and a failed request puts the last answer the server confirmed back on screen. One, two or three flames PATCHes `intensity` 1 to 3 (the server refuses 4); each 1 to 5 dot PATCHes `instructor_score` / `class_score` / `studio_score` (a repeat tap on the same value sends nothing); the comment (1000 characters; its hint "Other members will see this." is 14sp, a step under the 18sp the member types in, and sits on the rule) is PATCHed when the field loses focus and on Done if changed. In the tonal well every control is outlined until picked (transparent, MossLight edge, Ink type) and a pick fills Moss with Stone type, so the only filled shapes are the member's answers. The comment field starts ONE line tall with its text sitting on the rule, grows a line at a time to three as the member types, and scrolls inside those three lines from there to the 1000-character cap; its label, rule and counter are Charcoal at rest and Moss on focus. Done is a `SettlingCta`: the Moss pill with a Lime arrow well, which on tap dismisses the keyboard, presses in, fires the confirm haptic and settles as "FEEDBACK SAVED" with a check popped into the well, never changing colour (pills and dots fire the selection haptic). A save that takes longer than about 220ms (a changed comment over a slow link) sweeps a soft highlight across the pill until it lands; a quick one shows nothing. On Home and Reservations the settled pill shows for about 650ms before the card closes; on the intensity step the "Done" link becomes "Feedback saved" with a check for the same beat. Score rows are labelled from the saved review (folded class type, brand) rather than the row's raw session name. Done closes the card; a card that answered a second booking of an already reviewed combination receives the existing review (HTTP 200) and continues editing it. A 409 `not_eligible` collapses the card to "This class can't be reviewed."
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCard.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewViewModel.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ChoicePills.kt (`ChoicePills` `leading` and `showLabels`, `ScoreDots`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewMarks.kt (`AgainMark`, `IntensityFlames`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/theme/AppColors.kt (`MarkAmber`, `MarkRed`, `MarkBang`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Inputs.kt (`ArcanaMultilineTextField`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/Buttons.kt (`SettlingCta`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewSaveNotice.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewDrafts.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/networking/ReviewApi.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/ReviewDto.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/review/ReviewViewModelTest.kt
- **Platforms:** shared

### REVIEW-02 — Edit mode for an existing review
- **Steps:** Open class detail for a class whose combination the member already reviewed (any booking of it), or tap Edit on a card's "Your feedback is saved." line.
- **Expected:** Every control shows at once with the saved answers selected (again pills, intensity pills, the three score rows, the comment); each change PATCHes the review that already exists (never a second POST) and fires `review_edited`; Done saves a changed comment and the card stays fully open with its Moss pill settled as "FEEDBACK SAVED" and a check in the well (there is no collapsed "saved" line and no Edit link any more). `review_completed` never fires from edit mode.
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCard.kt (`EditBody`, `CommentAndDone`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewViewModel.kt (`settled`, `doneCount`, `keepOpen`), sharedLogic/src/commonTest/kotlin/org/arcana/mobile/review/ReviewViewModelTest.kt
- **Platforms:** shared

### REVIEW-03 — Feedback feed: header stats, anonymous items, keyset paging
- **Steps:** Open the feed from any entry point (Discover's Member feedback card for the unscoped feed; a studio page, a location link, an instructor sheet, or a class detail link for a scope). Scroll to the end of a scope with more than 20 reviews; pull to refresh; stop the server and pull again.
- **Expected:** A pushed screen (tab bar hidden, close X, system back closes it) titled with the scope label from the server ("Member feedback", the brand, "<Brand> · <Location>", the class type, the instructor) with, as the first card of a scoped feed that has MORE THAN ONE review (one review is its own average, so it shows none), the scope's averages in the same tonal well and the SAME five columns as the reviews under it (`ReviewAverageStrip`), headed "AVERAGE OF 6 REVIEWS": Again as ONE mark, the answer the reviews add up to with a yes counting whole and a maybe half (`againOnBalance`: two thirds or more is the green check, one third or less the red minus, anything between the amber warning; there is no percentage and no ring), Intensity as one to three flames to the rounded average, and Instructor / Class / Studio as one-decimal numerals ("4.8"); a column with no data keeps its place with the small hollow ring, and a screen reader hears the old sentence ("6 reviews · 5 would take a class again · usually Hard · Instructors 4.8 · Classes 4.5 · Studio 4.7"). There is no bullet-separated text line any more. The unscoped all-studios feed shows NO averages card (averages pooled across every studio describe nothing), and neither does a scope with zero reviews or one. The loading skeleton is card-shaped. Items are newest first, each its own tonal-well card 12dp apart with no rules between them: "SEP 12 · ARCANA MEMBER", brand and location, class type with instructor, the comment when present, then a hairline and `ReviewStatStrip`: five columns that never move (Again, Intensity, Instructor, Class, Studio; Intensity is half as wide again as the others so three flames fit, the same on every card) so answers compare down the feed. Each value sits OVER its label. Again is the mark from the card (green check emoji = yes, amber warning triangle = maybe, red minus disc = no; REVIEW-01), intensity one to three fire emoji in a row (Easy one, Hard three; Apple's emoji on iOS, Noto's on Android), the three scores League Spartan numerals; the mark and the flames are decorative; an unanswered column keeps its place with a small hollow ring. A screen reader hears the strip as one sentence ("Would take it again · Hard · Instructor 5 · Class 4 · Studio 4"). No member name, initials or email ever appears. A LOCATION feed whose brand has more reviews than the location shows an "ALL LOCATIONS · N REVIEWS" link under its stats line that pushes the brand's feed (`feedback_feed_opened` with `scope_type=brand`, `source=feed`); no other scope shows it. Twenty per page from `GET reviews/?<scope>=&cursor=`, loading the next within ten rows of the end behind one shimmer row, deduplicated by id, stopping at a null cursor, where the shared `EndOfListMarker` reads "THAT'S EVERY REVIEW" as one centred caps line with no dots above it, sitting about 20dp under the last card and 8dp above the bottom safe area (the feed passes its own tighter `contentPadding`; the Book tab and Search keep the roomier default) (absent while a page is loading or has failed); a failed page keeps the rows and offers an `InlineError` retry. Empty scope: "No feedback here yet." Cold failure: `FullScreenError` centred on the whole screen with the close X and the title drawn over it and still live, at the same height as Home's (ERR-06); failed refresh: the refresh-failed toast over the kept items. Scrolled away from the top, the feed shows the Book tab's jump chevron at the top centre of the list (`JumpToTop`); tapping it animates back to the first card and the chevron leaves. Tapping an item opens the studio page (`source=feed`).
- **Source:** sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/FeedbackFeedScreen.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewStatStrip.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/review/ReviewMarks.kt (`AgainMark`, `IntensityFlames`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/ErrorState.kt (`FullScreenError`, `chromeBottom`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/ui/TonalWell.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/FeedbackFeedViewModel.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/ReviewCopy.kt (`scopeStatsLine`, `showsAverages`, `averagesHeading`, `againScore`, `againOnBalance`, `usualIntensity`, `reviewAnswersLine`, `reviewDateLabel`), sharedUI/src/commonMain/kotlin/org/arcana/mobile/schedule/ScheduleScreen.kt (`EndOfListMarker`, `JumpToTop`), sharedLogic/src/commonMain/kotlin/org/arcana/mobile/review/FeedbackScope.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/review/FeedbackFeedViewModelTest.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/review/ReviewCopyTest.kt
- **Platforms:** shared

### REVIEW-04 — The feed is reachable from every tab and keeps that tab's stack
- **Steps:** Open a feed from class detail reached via Home, via the Book tab, via You → Reservations, and from Discover; tap an item to open its studio page; tap "See schedule" there; go back.
- **Expected:** `FeedbackFeed(scopeType, scopeValue, label, source)` is registered in the Android NavHost and in all four iOS tab graphs, and the studio page is registered wherever the feed is, so an item always opens its studio page on the current tab's stack (`studio_page_viewed` with `source=feed`). On iOS the native bar hides on both pushed screens and returns at the tab root; "See schedule" from a studio page reached this way shows the Book tab and leaves the page on its own stack (DISCOVER-03). `$screen: FeedbackFeed` fires once per open on both shells.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/navigation/ArcanaDestinations.kt, sharedUI/src/commonMain/kotlin/org/arcana/mobile/App.kt (`feedbackRoute`, `currentScreenName`), sharedUI/src/iosMain/kotlin/org/arcana/mobile/shell/TabRoots.kt (`feedbackDestinations`)
- **Platforms:** shared

### REVIEW-05 — New DTO fields default so older payloads still parse
- **Steps:** Point the app at a server without the reviews release (or replay stored payloads through the DTO tests).
- **Expected:** `review_state` defaults to `ineligible` (no pills, no "Reviewed"), `review_prompt` to null (no Home card), class detail's `class_type_review_count` to 0, `my_review` and `review_booking_id` to null, instructors' `profile_id` to null (row not tappable) and `review_count` to 0, the directory's `feedback` to an empty block (no row) and every studio page count to 0 (no links). Nothing crashes and nothing review-related renders.
- **Source:** sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/BookingDto.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/ScheduleDto.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/DiscoverDto.kt, sharedLogic/src/commonMain/kotlin/org/arcana/mobile/data/ReviewDto.kt, sharedLogic/src/commonTest/kotlin/org/arcana/mobile/review/ReviewDtoTest.kt
- **Platforms:** shared
