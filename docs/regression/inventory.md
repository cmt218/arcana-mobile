...
- **Expected:** `onSeeAllBookings` fires, navigating to the My Bookings screen (regardless of whether there are 0, few, or many upcoming bookings — the link always renders in the Success state). The `TextLink` itself now dims to 70% alpha while pressed (no scale) instead of showing no press feedback at all — every other `TextLink` call site (My Bookings' "Cancel", Profile's "Manage", Signup's "Continue anyway"/"Log in") gets the same behavior for free, since it's one shared component.
