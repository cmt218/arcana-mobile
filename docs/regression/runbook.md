...
`docs/regression/inventory.md` (226 entries at last count) on all three
...
For every **Source:** path listed across all 226 inventory entries, verify
...
should yield **82 unique paths from 498 comma-split tokens across 226 Source
...
per-device verdict. Applicable counts on today's inventory (226 entries: 198
`shared`, 11 `iOS-only`, 1 `iOS26-only`, 1 `iOS18-only`, 15 `Android-only`)
are **210** for iOS 26, **210** for iOS 18, **213** for Android — recompute
...
per-item results (226 entries × 3 devices)</summary>` block so it never
