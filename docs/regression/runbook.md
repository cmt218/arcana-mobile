...
`docs/regression/inventory.md` (218 entries at last count) on all three
...
For every **Source:** path listed across all 218 inventory entries, verify
...
should yield **67 unique paths from 434 comma-split tokens across 218 Source
lines** (one Source line per entry, which is also a free cross-check on the
entry count) — verified 2026-08-11 and again 2026-08-15. `self_audit.sh`
...
per-device verdict. Applicable counts on today's inventory (218 entries: 192
`shared`, 11 `iOS-only`, 1 `iOS26-only`, 1 `iOS18-only`, 13 `Android-only`)
are **204** for iOS 26, **204** for iOS 18, **205** for Android — recompute
...
per-item results (218 entries × 3 devices)</summary>` block so it never
