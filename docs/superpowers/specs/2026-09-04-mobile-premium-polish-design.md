...
**Vignette:** the prototype's shader, as a radial brush over the mesh: `smoothstep(0.30, 0.95, d)` with `d` the distance from the centre in screen widths, `#A6B277` at 10% alpha where it saturates (from 0.95 widths out, so every corner of a phone). The centre points are a deep olive inside a light-olive perimeter, so the field reads as a gradient that breathes as the points drift rather than a flat slab; the perimeter is off Stone enough to avoid a white edge halo.
