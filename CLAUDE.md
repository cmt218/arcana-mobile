...
- `auth/SecureStorage` is an `expect/actual` class: `EncryptedSharedPreferences` on Android, iOS Keychain via CoreFoundation/Security on iOS. `TokenStorage` wraps it with typed `accessToken`/`refreshToken` properties. **iOS `save()` writes in place** (`SecItemUpdate`, adding only when the key is new). It must never delete-then-add again: items are `WhenUnlockedThisDeviceOnly`, so a write from a locked device is rejected, and with rotating refresh tokens rewriting every few minutes from the background, the old delete-first order could destroy the token permanently. A rejected write now leaves the previous value intact.
- **Only an affirmative server rejection may end a session** (Cole, 2026-08-16). In `refreshTokens`, a 401/403 from the refresh endpoint calls `forceLogout("refresh_error")`; *every* other failure — transport error, 5xx/429, unparseable body, and a refresh token that reads back null — keeps the session, reports `auth_refresh_failed`, and lets the next request retry. `refresh_error` is now the only `forced_logout` cause. The trade is deliberate: a genuinely unrecoverable token leaves an authed shell whose requests fail, rather than an unexplained logout, and the member's exit is Profile → Sign out. Do not "fix" a storage failure back into a logout.
...
- Kotlin: 2.3.10
- Compose Multiplatform: 1.12.0 (concurrent/parallel rendering is ON by default on iOS since 1.11.0)
- Koin: 4.2.0
- Ktor: 3.1.2
- kotlinx-datetime: 0.7.1
- Navigation Compose: 2.10.0-alpha02 (JetBrains CMP port — pinned to a version matched to the CMP release in the CMP CHANGELOG; do not bump independently of `composeMultiplatform`)
...
- Material3 (`org.jetbrains.compose.material3`): 1.12.0-alpha03 — the multiplatform material3 port publishes **alpha-only** since it decoupled from CMP releases (no stable exists past 1.8.x); always use the exact version listed in the CMP release notes. We have shipped alpha material3 from day one; this is normal, not a red flag.
- Android min/target SDK: 29/36 (minSdk raised from 24 on 2026-09-06: no member below Android 11 in 90 days of PostHog, and `drawVertices`, which the atmosphere needs, is hardware-accelerated only from 29; compileSdk 37 — required by lifecycle 2.11.0's Jetpack delegate; AGP 9.1 warns it was tested up to 36.1, warning-only)
...
**Text on the atmosphere uses `Charcoal`, not `Ash`.** `Ash` (#6B6E5F) reads at about 3.2:1 over the olive atmosphere and `Ash2` under 2:1, so secondary text placed directly on a Stone root (section rules, overlines, captions, empty-state copy, the full-class label) is `Charcoal`; `Ash` stays for text on lifted surfaces (cards, popovers, sheets, the tab pill) and for hairlines and outlines, and `Ash2` for tertiary text on those surfaces only. `Overline` and `Caption` default to `Charcoal`.
...
**Splash** (`SplashScreen.kt` + `ui/SplashWordmark.kt`): full-screen Compose Canvas that redraws the Arcana wordmark (grid from `theme/WordmarkGrid.kt`, embedded const, no I/O) row by row from the top, each row flashing lime before it settles to stone, then breath-pulses. Sits on MossDeep with a radial vignette fading to Ink at the corners. Timing: `SPLASH_WORDMARK_SETTLED_MS` (the redraw, 0.82 s) plus a 0.45 s hold is `SPLASH_MIN_DISPLAY_MS`, before the exit: a 0.55 s ease-in-out fade with the settled mark holding still (`SPLASH_EXIT_MS`; `App.kt` on Android, `ArcanaShell.swift` through `IosShellBridge` on iOS). A zoom was tried and rejected by Cole. On iOS the live splash fades in place with an opacity modifier and is dropped afterwards: as a SwiftUI removal transition in the `ZStack` it played as a one-frame cut. `SplashWordmark`'s clock must not be keyed on its size, or any resize restarts the redraw and blanks the wordmark. It is a fixed timer sized to cover Home's first fetch (docs/perf/README.md "Splash length"); `SplashWordmarkTest` holds the lead-in to ending on the same settled wordmark. On Android the system launch screen is themed to the same MossDeep with no icon (`Theme.Arcana.Launch`), so it reads as the start of the splash. The canonical reference grid JSON is in `composeResources/files/wordmark-grid.json`; if it ever changes, regenerate the const in `WordmarkGridData.kt` from the file.
